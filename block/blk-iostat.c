...
static void __iostat_issue(struct rq_qos *rqos, struct iostat_gq *is,
			   struct request *req)
{
...
static void __iostat_done(struct rq_qos *rq_qos, struct iostat_gq *is,
			  struct request *req)
{
	struct iostat_data *stat;
	int op = iostat_op(req);
...
static void iostat_exit(struct rq_qos *rqos)
{
	struct iostat_queue *isq =
		container_of(rqos, struct iostat_queue, rqos);
...
static void iostat_sum(struct blkcg_gq *blkg, struct iostat_data *sum,
		       bool meta)
{
...
iostat_pd_alloc(gfp_t gfp, struct request_queue *q, struct blkcg *blkcg)
{
	struct iostat_gq *is;
	...
	is->stat.data =
		__alloc_percpu_gfp(sizeof(struct iostat_data),
     ...
     */
	strlcpy(is->disk_name, kobject_name(q->kobj.parent), DISK_NAME_LEN);
	return &is->pd;
out_free:
	if (is->stat.data)
		free_percpu(is->stat.data);
