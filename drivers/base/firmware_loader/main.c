...
static int alloc_lookup_fw_priv(const char *fw_name, struct firmware_cache *fwc,
				struct fw_priv **fw_priv, void *dbuf,
				size_t size, size_t offset, u32 opt_flags)
{
	struct fw_priv *tmp;
...
static void __free_fw_priv(struct kref *ref) __releases(&fwc->lock)
{
	...
	pr_debug("%s: fw-%s fw_priv=%p data=%p size=%u\n", __func__,
		 fw_priv->fw_name, fw_priv, fw_priv->data,
...
int fw_grow_paged_buf(struct fw_priv *fw_priv, int pages_needed)
{
	...
	if (fw_priv->page_array_size < pages_needed) {
		int new_array_size =
			max(pages_needed, fw_priv->page_array_size * 2);
		...
		memset(&new_pages[fw_priv->page_array_size], 0,
		       sizeof(void *) *
...
int fw_map_paged_buf(struct fw_priv *fw_priv)
{
	...
	vunmap(fw_priv->data);
	fw_priv->data =
		vmap(fw_priv->pages, fw_priv->nr_pages, 0, PAGE_KERNEL_RO);
	if (!fw_priv->data)
		return -ENOMEM;
			    ...
			    size_t in_size, const void *in_buffer)
{
	...
	if (fw_priv->data)
		return fw_decompress_xz_single(dev, fw_priv, in_size,
					       in_buffer);
	else
...
static int fw_get_filesystem_firmware(
	struct device *device, struct fw_priv *fw_priv, const char *suffix,
	int (*decompress)(struct device *dev, struct fw_priv *fw_priv,
			  size_t in_size, const void *in_buffer))
{
	size_t size;
	int i, len;
	...
	for (i = 0; i < ARRAY_SIZE(fw_path); i++) {
		...
		len = snprintf(path, PATH_MAX, "%s/%s%s", fw_path[i],
					      ...
					      const char *name)
{
	...
	fwn = devres_find(dev, fw_name_devm_release, fw_devm_match,
			  (void *)name);
	return fwn;
}
...
static int _request_firmware_prepare(struct firmware **firmware_p,
				     ...
				     void *dbuf, size_t size, size_t offset,
				     u32 opt_flags)
{
...
static int _request_firmware(const struct firmware **firmware_p,
			     const char *name, struct device *device, void *buf,
			     size_t size, size_t offset, u32 opt_flags)
{
	...
	ret = _request_firmware_prepare(&fw, name, device, buf, size, offset,
					opt_flags);
					 ...
					 void *context))
{
	...
	fw_work->cont = cont;
	fw_work->opt_flags =
		FW_OPT_NOWAIT | (uevent ? FW_OPT_UEVENT : FW_OPT_USERHELPER);
...
static void dev_cache_fw_image(struct device *dev, void *data)
{
	...
	devres_for_each_res(dev, fw_name_devm_release, devm_name_match,
			    &fw_cache, dev_create_fw_entry, &todo);
...
static void __device_uncache_fw_images(void)
{
	...
	while (!list_empty(&fwc->fw_names)) {
		fce = list_entry(fwc->fw_names.next, struct fw_cache_entry,
				 list);
		list_del(&fce->list);
		spin_unlock(&fwc->name_lock);
