...
static void i2c_gpio_init_pinctrl_recovery(struct i2c_adapter *adap)
{
	...
	if (!bri->pins_default) {
		...
		if (IS_ERR(bri->pins_default)) {
			dev_dbg(dev, PINCTRL_STATE_DEFAULT
...
static int i2c_device_probe(struct device *dev)
{
	...
	client->devres_group_id =
		devres_open_group(&client->dev, NULL, GFP_KERNEL);
					 ...
					 struct i2c_board_info const *info)
{
	...
	if (!client->init_irq)
		client->init_irq = i2c_dev_irq_from_resources(
			info->resources, info->num_resources);
...
static struct attribute *i2c_adapter_attrs[] = { &dev_attr_name.attr,
...
static void i2c_scan_static_board_info(struct i2c_adapter *adapter)
{
	...
	list_for_each_entry (devinfo, &__i2c_board_list, list) {
		if (devinfo->busnum == adapter->nr &&
		    IS_ERR(i2c_new_client_device(adapter,
						 &devinfo->board_info)))
			dev_err(&adapter->dev,
...
static int i2c_host_notify_irq_map(struct irq_domain *h, unsigned int virq,
...
int i2c_add_adapter(struct i2c_adapter *adapter)
{
	...
	mutex_lock(&core_lock);
	id = idr_alloc(&i2c_adapter_idr, adapter, __i2c_first_dynamic_bus_num,
		       0, GFP_KERNEL);
	mutex_unlock(&core_lock);
	if (WARN(id < 0, "couldn't get idr"))
		return id;
			  ...
			  bool use_defaults)
{
	...
	i2c_parse_timing(dev, "i2c-scl-internal-delay-ns", &t->scl_int_delay_ns,
			 0, u);
				...
				int num)
{
	...
	if (q->flags & I2C_AQ_COMB) {
		...
		if (num == 2) {
			if (q->flags & I2C_AQ_COMB_WRITE_FIRST &&
			    msgs[0].flags & I2C_M_RD)
				return i2c_quirk_error(
					adap, &msgs[0],
	...
	for (i = 0; i < num; i++) {
		...
		if (msgs[i].flags & I2C_M_RD) {
			if (do_len_check &&
			    i2c_quirk_exceeded(len, q->max_read_len))
			...
			if (do_len_check &&
			    i2c_quirk_exceeded(len, q->max_write_len))
				return i2c_quirk_error(adap, &msgs[i],
						       "msg too long");
		      ...
		      struct i2c_device_identity *id)
{
	...
	raw_id.block[0] = 3;
	ret = i2c_smbus_xfer(adap, I2C_ADDR_DEVICE_ID, 0, I2C_SMBUS_READ,
			     client->addr << 1, I2C_SMBUS_I2C_BLOCK_DATA,
			     &raw_id);
	if (ret)
		return ret;
...
{
	return i2c_smbus_xfer(adap, addr, 0, I2C_SMBUS_READ, 0, I2C_SMBUS_QUICK,
			      NULL) >= 0;
}
