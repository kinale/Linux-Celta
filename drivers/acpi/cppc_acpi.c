...
#define show_cppc_data(access_fn, struct_name, member_name)                    \
	static ssize_t show_##member_name(                                     \
...
static struct attribute *cppc_attrs[] = {
	&feedback_ctrs.attr,   &reference_perf.attr,
	&wraparound_time.attr, &highest_perf.attr,
	&lowest_perf.attr,     &lowest_nonlinear_perf.attr,
	&nominal_perf.attr,    &nominal_freq.attr,
	&lowest_freq.attr,     NULL
};
...
static int send_pcc_cmd(int pcc_ss_id, u16 cmd)
{
	...
	if (pcc_ss_data->pcc_mpar) {
		if (pcc_ss_data->mpar_count == 0) {
			time_delta = ktime_ms_delta(
				ktime_get(), pcc_ss_data->last_mpar_reset);
			if ((time_delta < 60 * MSEC_PER_SEC) &&
			    pcc_ss_data->last_mpar_reset) {
				pr_debug(
					"PCC cmd for subspace %d not sent due to MPAR limit",
					pcc_ss_id);
				ret = -EIO;
				goto end;
			}
...
static int acpi_get_psd(struct cpc_desc *cpc_ptr, acpi_handle handle)
{
	...
	status = acpi_extract_package(&(psd->package.elements[0]), &format,
				      &state);
	if (ACPI_FAILURE(status)) {
...
static int register_pcc_channel(int pcc_ss_idx)
{
	...
	if (pcc_ss_idx >= 0) {
		...
		pcc_data[pcc_ss_idx]->pcc_comm_addr = acpi_os_ioremap(
...
static int pcc_data_alloc(int pcc_ss_id)
{
	...
	if (pcc_data[pcc_ss_id]) {
		...
		pcc_data[pcc_ss_id] =
			kzalloc(sizeof(struct cppc_pcc_data), GFP_KERNEL);
		if (!pcc_data[pcc_ss_id])
			return -ENOMEM;
		pcc_data[pcc_ss_id]->refcount++;
	}
...
static bool is_cppc_supported(int revision, int num_ent)
{
	...
	if (expected_num_ent != num_ent) {
		pr_debug(
			"Firmware exports %d entries. Expected: %d for CPPC rev:%d\n",
...
int acpi_cppc_processor_probe(struct acpi_processor *pr)
{
		...
		if (cpc_obj->type == ACPI_TYPE_INTEGER) {
			...
			if (gas_t->space_id == ACPI_ADR_SPACE_PLATFORM_COMM) {
				...
				if (gas_t->address) {
					...
					addr = ioremap(gas_t->address,
						       gas_t->bit_width / 8);
					if (!addr)
						goto out_free;
					cpc_ptr->cpc_regs[i - 2].sys_mem_vaddr =
						addr;
				}
			} else if (gas_t->space_id ==
				   ACPI_ADR_SPACE_SYSTEM_IO) {
				if (gas_t->access_width < 1 ||
				    gas_t->access_width > 3) {
					/*
				...
				if (gas_t->address & OVER_16BTS_MASK) {
					...
					pr_debug(
						"Invalid IO port %llu for SystemIO register\n",
						gas_t->address);
					goto out_free;
				}
			} else {
	...
	if (pcc_subspace_id >= 0 &&
		...
		init_waitqueue_head(
			&pcc_data[pcc_subspace_id]->pcc_write_wait_q);
	}
		...
		if (pcc_data[pcc_ss_id]->pcc_channel_acquired) {
			...
			if (!pcc_data[pcc_ss_id]->refcount) {
				pcc_mbox_free_channel(
					pcc_data[pcc_ss_id]->pcc_channel);
...
int cppc_set_perf(int cpu, struct cppc_perf_ctrls *perf_ctrls)
{
	...
	if (CPC_IN_PCC(desired_reg)) {
		if (down_write_trylock(
			    &pcc_ss_data->pcc_lock)) { /* BEGIN Phase-II */
