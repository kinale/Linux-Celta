...
static void intel_pstate_set_itmt_prio(int cpu)
{
	...
	if (cppc_perf.highest_perf == CPPC_MAX_PERF)
		cppc_perf.highest_perf = HWP_HIGHEST_PERF(
			READ_ONCE(all_cpu_data[cpu]->hwp_cap_cached));
...
static void intel_pstate_hybrid_hwp_adjust(struct cpudata *cpu)
{
	...
	cpu->pstate.max_freq =
		rounddown(cpu->pstate.max_pstate * scaling, perf_ctl_scaling);

	cpu->pstate.max_pstate_physical =
		DIV_ROUND_UP(perf_ctl_max_phys * perf_ctl_scaling, scaling);
...
static int min_perf_pct_min(void)
{
	...
	return turbo_pstate ? (cpu->pstate.min_pstate * 100 / turbo_pstate) : 0;
}
...
static const unsigned int epp_values[] = { HWP_EPP_PERFORMANCE,
...
static ssize_t
show_energy_performance_available_preferences(struct cpufreq_policy *policy,
					      char *buf)
{
	int i = 0;
	int ret = 0;
...
static ssize_t
store_energy_performance_preference(struct cpufreq_policy *policy,
...
static ssize_t show_energy_performance_preference(struct cpufreq_policy *policy,
						  char *buf)
{
...
static void intel_pstate_get_hwp_cap(struct cpudata *cpu)
{
	...
	if (scaling != cpu->pstate.perf_ctl_scaling) {
		...
		cpu->pstate.max_freq =
			rounddown(cpu->pstate.max_freq, perf_ctl_scaling);
		cpu->pstate.turbo_freq =
			rounddown(cpu->pstate.turbo_freq, perf_ctl_scaling);
	}
}
...
static ssize_t show_status(struct kobject *kobj, struct kobj_attribute *attr,
			   char *buf)
{
	ssize_t ret;
...
static ssize_t show_turbo_pct(struct kobject *kobj, struct kobj_attribute *attr,
			      char *buf)
{
	struct cpudata *cpu;
...
static ssize_t show_no_turbo(struct kobject *kobj, struct kobj_attribute *attr,
			     char *buf)
{
	ssize_t ret;
			      ...
			      const char *buf, size_t count)
{
	...
	if (global.no_turbo) {
		struct cpudata *cpu = all_cpu_data[0];
		int pct =
			cpu->pstate.max_pstate * 100 / cpu->pstate.turbo_pstate;
				  ...
				  const char *buf, size_t count)
{
	...
	global.min_perf_pct =
		clamp_t(int, input, min_perf_pct_min(), global.max_perf_pct);
...
static struct attribute *intel_pstate_attributes[] = { &status.attr,
						       &no_turbo.attr, NULL };
...
static void __init intel_pstate_sysfs_expose_params(void)
{
	...
	intel_pstate_kobject = kobject_create_and_add(
		"intel_pstate", &cpu_subsys.dev_root->kobj);
...
{
	struct cpudata *cpudata = container_of(to_delayed_work(work),
					       struct cpudata, hwp_notify_work);
...
static u64 atom_get_val(struct cpudata *cpudata, int pstate)
{
	...
	vid_fp = cpudata->vid.min +
		 mul_fp(int_tofp(pstate - cpudata->pstate.min_pstate),
			cpudata->vid.ratio);
...
static int silvermont_get_scaling(void)
{
	...
	static int silvermont_freq_table[] = { 83300, 100000, 133300, 116700,
					       80000 };
...
static int airmont_get_scaling(void)
{
	...
	static int airmont_freq_table[] = { 83300,  100000, 133300,
					    116700, 80000,  93300,
					    90000,  88900,  87500 };
...
static void atom_get_vid(struct cpudata *cpudata)
{
	...
	cpudata->vid.ratio = div_fp(cpudata->vid.max - cpudata->vid.min,
...
static inline int32_t get_target_pstate(struct cpudata *cpu)
{
	...
	busy_frac =
		div_fp(sample->mperf << cpu->aperf_mperf_shift, sample->tsc);
...
static void intel_pstate_adjust_pstate(struct cpudata *cpu)
{
	...
	trace_pstate_sample(mul_ext_fp(100, sample->core_avg_perf),
			    fp_toint(sample->busy_scaled), from,
			    cpu->pstate.current_pstate, sample->mperf,
			    sample->aperf, sample->tsc, get_avg_frequency(cpu),
					    ...
					    unsigned int policy_max)
{
	...
	if (policy_max == policy_min) {
		...
		min_policy_perf =
			clamp_t(int32_t, min_policy_perf, 0, max_policy_perf);
	}
	...
	if (per_cpu_limits) {
		...
		cpu->min_perf_ratio =
			min(cpu->min_perf_ratio, cpu->max_perf_ratio);
	}
				...
				int old_pstate)
{
	...
	sample = &cpu->sample;
	trace_pstate_sample(trace_type, 0, old_pstate,
			    cpu->pstate.current_pstate, sample->mperf,
			    sample->aperf, sample->tsc, get_avg_frequency(cpu),
			    fp_toint(cpu->iowait_boost * 100));
}
				       ...
				       int target_pstate, bool fast_switch)
{
	...
	target_pstate = intel_pstate_prepare_request(cpu, target_pstate);
	if (hwp_active) {
		int max_pstate = policy->strict_target ? target_pstate :
							       cpu->max_perf_ratio;
				...
				unsigned int target_freq, unsigned int relation)
{
	...
	default:
		target_pstate =
			DIV_ROUND_CLOSEST(freqs.new, cpu->pstate.scaling);
		break;
	}

	target_pstate =
		intel_cpufreq_update_pstate(policy, target_pstate, false);
				      ...
				      unsigned long capacity)
{
	...
	if (target_perf < capacity)
		target_pstate =
			DIV_ROUND_UP(cap_pstate * target_perf, capacity);
...
static ssize_t intel_pstate_show_status(char *buf)
{
	...
	return sprintf(buf, "%s\n",
		       intel_pstate_driver == &intel_pstate ? "active" :
								    "passive");
}
...
{
	if (!pstate_funcs.get_max() || !pstate_funcs.get_min() ||
...
static bool __init intel_pstate_no_acpi_pss(void)
{
	...
	for_each_possible_cpu (i) {
		...
		status =
			acpi_evaluate_object(pr->handle, "_PSS", NULL, &buffer);
