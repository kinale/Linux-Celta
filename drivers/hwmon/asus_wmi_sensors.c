...
enum asus_wmi_source { SIO = 0x1, EC = 0x2 };

static enum hwmon_sensor_types asus_data_types[] = {
	[VOLTAGE] = hwmon_in,	  [TEMPERATURE_C] = hwmon_temp,
	[FAN_RPM] = hwmon_fan,	  [CURRENT] = hwmon_curr,
...
{
	union acpi_object name_obj, data_type_obj, location_obj, source_obj,
		type_obj;
	struct acpi_buffer output = { ACPI_ALLOCATE_BUFFER, NULL };
	u32 args[] = { index, 0 };
	union acpi_object *obj;
...
static int asus_wmi_update_buffer(int source)
{
	...
	return asus_wmi_call_method(ASUSWMI_METHODID_UPDATE_BUFFER, args,
				    &output);
}
...
static int
asus_wmi_update_values_for_source(u8 source,
...
static int
asus_wmi_get_cached_value_or_update(const struct asus_wmi_sensor_info *sensor,
				    struct asus_wmi_sensors *sensor_data,
				    u32 *value)
{
	int ret = 0;
	...
	if (time_after(jiffies,
		       sensor_data->wmi.source_last_updated[sensor->source] +
			       HZ)) {
		ret = asus_wmi_update_buffer(sensor->source);
		if (ret)
			goto unlock;

		ret = asus_wmi_update_values_for_source(sensor->source,
							sensor_data);
		if (ret)
			goto unlock;
					   ...
					   struct asus_wmi_sensors *sensor_data)
{
	...
	sensor_data->wmi.info_by_id =
		devm_kcalloc(dev, sensor_data->wmi.sensor_count,
			     sizeof(*sensor_data->wmi.info_by_id), GFP_KERNEL);
	...
	dev_dbg(dev, "board has %d sensors", sensor_data->wmi.sensor_count);

	hwdev = devm_hwmon_device_register_with_info(
		dev, "asus_wmi_sensors", sensor_data, chip_info, NULL);
