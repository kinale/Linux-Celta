...
#define MAKE_SENSOR_ADDRESS(size_i, bank_i, index_i)                           \
	{                                                                      \
		.size = size_i, .bank = bank_i, .index = index_i,              \
	}
...
static struct asus_wmi_data sensors_board_PW_X570_P = {
	.known_board_sensors = { SENSOR_TEMP_CHIPSET, SENSOR_TEMP_CPU,
				 SENSOR_TEMP_MB, SENSOR_TEMP_T_SENSOR,
...
static struct asus_wmi_data sensors_board_R_C8H = {
	.known_board_sensors = { SENSOR_TEMP_CHIPSET, SENSOR_TEMP_CPU,
				 SENSOR_TEMP_MB, SENSOR_TEMP_T_SENSOR,
				 SENSOR_TEMP_VRM, SENSOR_TEMP_WATER_IN,
				 SENSOR_TEMP_WATER_OUT, SENSOR_FAN_CPU_OPT,
				 SENSOR_FAN_CHIPSET, SENSOR_FAN_WATER_FLOW,
				 SENSOR_CURR_CPU, SENSOR_MAX },
};
...
static int
asus_wmi_ec_get_cached_value_or_update(struct asus_wmi_sensors *sensor_data,
				       int sensor_index, long *value)
{
...
static int asus_wmi_ec_hwmon_read(struct device *dev,
				  ...
				  int channel, long *val)
{
	...
	*val = asus_wmi_ec_scale_sensor_value(
		value, known_ec_sensors[info_index].type);
					      ...
					      const enum known_ec_sensor *bsi)
{
	...
	asus_wmi_ci = devm_kcalloc(dev, nr_types + 1, sizeof(*asus_wmi_ci),
				   GFP_KERNEL);
	if (!asus_wmi_ci)
		return -ENOMEM;
	...
	hwdev = devm_hwmon_device_register_with_info(
		dev, "asus_wmi_ec_sensors", sensor_data, chip_info, NULL);
