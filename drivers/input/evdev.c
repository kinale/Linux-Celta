...
static bool __evdev_is_filtered(struct evdev_client *client, unsigned int type,
				unsigned int code)
{
	unsigned long *mask;
	size_t cnt;
...
static void evdev_pass_values(struct evdev_client *client,
			      ...
			      unsigned int count, ktime_t *ev_time)
{
	...
	if (wakeup)
		wake_up_interruptible_poll(&client->wait, EPOLLIN | EPOLLOUT |
								  EPOLLRDNORM |
								  EPOLLWRNORM);
}
...
{
	struct evdev_client *client =
		container_of(rp, struct evdev_client, rcu);
	unsigned int i;
	for (i = 0; i < EV_CNT; ++i)
...
static ssize_t evdev_read(struct file *file, char __user *buffer, size_t count,
			  loff_t *ppos)
{
	struct evdev_client *client = file->private_data;
	struct evdev *evdev = client->evdev;
			  ...
			  unsigned int maxlen, const void __user *p, int compat)
{
	...
	if (compat) {
		...
		for (i = 0; i < len / sizeof(compat_long_t); i++)
			if (copy_from_user((compat_long_t *)bits + i + 1 -
						   ((i % 2) << 1),
...
static int handle_eviocgbit(struct input_dev *dev, unsigned int type,
			    unsigned int size, void __user *p, int compat_mode)
{
	unsigned long *bits;
	int len;
...
static int evdev_handle_mt_request(struct input_dev *dev, unsigned int size,
				   int __user *ip)
{
...
static int evdev_set_mask(struct evdev_client *client, unsigned int type,
			  const void __user *codes, u32 codes_size, int compat)
{
...
/* must be called with evdev-mutex held */
static int evdev_get_mask(struct evdev_client *client, unsigned int type,
			  void __user *codes, u32 codes_size, int compat)
{
	unsigned long *mask;
	...
	if (cnt > 0) {
		mask = client->evmasks[type];
		if (mask) {
			error = bits_to_user(mask, cnt - 1, xfer_size, codes,
					     compat);
			if (error < 0)
				return error;
		} else {
...
static long evdev_do_ioctl(struct file *file, unsigned int cmd, void __user *p,
			   int compat_mode)
{
	struct evdev_client *client = file->private_data;
	struct evdev *evdev = client->evdev;
	...
	case EVIOCGMASK: {
		...
		return evdev_get_mask(client, mask.type, codes_ptr,
	...
	case EVIOCSMASK: {
		...
		return evdev_set_mask(client, mask.type, codes_ptr,
				      mask.codes_size, compat_mode);
	}
...
#define EVIOC_MASK_SIZE(nr) ((nr) & ~(_IOC_SIZEMASK << _IOC_SIZESHIFT))
	...
	case EVIOCGPROP(0):
		return bits_to_user(dev->propbit, INPUT_PROP_MAX, size, p,
				    compat_mode);
	...
	case EVIOCGSW(0):
		return evdev_handle_get_val(client, dev, EV_SW, dev->sw, SW_MAX,
					    size, p, compat_mode);
	...
	if (_IOC_DIR(cmd) == _IOC_READ) {
		if ((_IOC_NR(cmd) & ~EV_MAX) == _IOC_NR(EVIOCGBIT(0, 0)))
			return handle_eviocgbit(dev, _IOC_NR(cmd) & EV_MAX,
						size, p, compat_mode);
...
#ifdef CONFIG_COMPAT
static long evdev_ioctl_compat(struct file *file, unsigned int cmd,
			       unsigned long arg)
{
