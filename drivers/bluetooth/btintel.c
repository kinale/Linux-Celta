...
int btintel_version_info(struct hci_dev *hdev, struct intel_version *ver)
{
	...
	bt_dev_info(hdev, "%s revision %u.%u build %u week %u %u", variant,
		    ver->fw_revision >> 4, ver->fw_revision & 0x0f,
			       ...
			       const void *param)
{
	while (plen > 0) {
		...
		skb = __hci_cmd_sync(hdev, 0xfc09, fragment_len + 1, cmd_param,
				     HCI_INIT_TIMEOUT);
		if (IS_ERR(skb))
			return PTR_ERR(skb);
				    ...
				    struct intel_version_tlv *version)
{
	...
	case 0x03:
		...
		break;
	default:
		bt_dev_err(hdev, "Unsupported image type(%02x)",
			   version->img_type);
		return -EINVAL;
	}
...
static int regmap_ibt_gather_write(void *context, const void *addr,
...
static bool btintel_firmware_version(struct hci_dev *hdev, u8 num, u8 ww, u8 yy,
				     const struct firmware *fw, u32 *boot_addr)
{
	const u8 *fw_ptr;
...
int btintel_download_firmware(struct hci_dev *hdev, struct intel_version *ver,
				   ...
				   u8 hw_variant, u8 sbe_type)
{
	...
	if (hw_variant <= 0x14) {
		...
		css_header_ver = get_unaligned_le32(fw->data + ECDSA_OFFSET +
						    CSS_HEADER_OFFSET);
		...
		if (sbe_type == 0x00) {
			...
			err = btintel_download_firmware_payload(
				hdev, fw, RSA_HEADER_LEN + ECDSA_HEADER_LEN);
			if (err)
				return err;
...
{
	u8 mask[11] = { 0x0a, 0x92, 0x02, 0x7f, 0x00, 0x00,
...
static int
btintel_reset_debug_features(struct hci_dev *hdev,
			     const struct intel_debug_features *features)
{
	u8 mask[11] = { 0x0a, 0x92, 0x02, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00 };
...
btintel_legacy_rom_get_fw(struct hci_dev *hdev, struct intel_version *ver)
{
	const struct firmware *fw;
	char fwname[64];
	int ret;
...
static int btintel_get_fw_name(struct intel_version *ver,
			       struct intel_boot_params *params, char *fw_name,
			       size_t len, const char *suffix)
{
	...
	case 0x0c: /* WsP */
		snprintf(fw_name, len, "intel/ibt-%u-%u.%s",
			 ...
			 le16_to_cpu(params->dev_revid), suffix);
		break;
	case 0x11: /* JfP */
	case 0x12: /* ThP */
	case 0x13: /* HrP */
	case 0x14: /* CcP */
		snprintf(fw_name, len, "intel/ibt-%u-%u-%u.%s",
			 ...
			 le16_to_cpu(ver->fw_revision), suffix);
		break;
	default:
		return -EINVAL;
	}
...
static int btintel_download_fw(struct hci_dev *hdev, struct intel_version *ver,
			       struct intel_boot_params *params,
			       u32 *boot_param)
{
	const struct firmware *fw;
				    ...
				    struct intel_version *ver)
{
	...
	err = btintel_get_fw_name(ver, &params, ddcname, sizeof(ddcname),
				  "ddc");
...
static int btintel_get_codec_config_data(struct hci_dev *hdev, __u8 link,
					 struct bt_codec *codec, __u8 *ven_len,
					 __u8 **ven_data)
{
	int err = 0;
...
void btintel_secure_send_result(struct hci_dev *hdev, const void *ptr,
				unsigned int len)
{
