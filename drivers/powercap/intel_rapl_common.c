...
#define PRIMITIVE_INFO_INIT(p, m, s, i, u, f)                                  \
	{                                                                      \
		.name = #p, .mask = m, .shift = s, .id = i, .unit = u,         \
		.flag = f                                                      \
	}

static void rapl_init_domains(struct rapl_package *rp);
static int rapl_read_data_raw(struct rapl_domain *rd, enum rapl_primitives prim,
			      bool xlate, u64 *data);
...
static struct rapl_primitive_info rpi[] = {
	...
	PRIMITIVE_INFO_INIT(FW_LOCK, POWER_LOW_LOCK, 31, RAPL_DOMAIN_REG_LIMIT,
			    ARBITRARY_UNIT, 0),
...
static int rapl_check_unit_core(struct rapl_package *rp, int cpu)
{
	...
	pr_debug("Core CPU %s energy=%dpJ, time=%dus, power=%duW\n", rp->name,
		 rp->energy_unit, rp->time_unit, rp->power_unit);
...
static int rapl_package_register_powercap(struct rapl_package *rp)
{
	...
	for (rd = rp->domains; rd < rp->domains + rp->nr_domains; rd++) {
		if (rd->id == RAPL_DOMAIN_PACKAGE) {
			...
			power_zone = powercap_register_zone(
				&rd->power_zone, rp->priv->control_type,
				rp->name, NULL, &zone_ops[rd->id], nr_pl,
				&constraint_ops);
	...
	for (rd = rp->domains; rd < rp->domains + rp->nr_domains; rd++) {
		...
		nr_pl = find_nr_power_limit(rd);
		power_zone = powercap_register_zone(
			&rd->power_zone, rp->priv->control_type, rd->name,
			parent, &zone_ops[rd->id], nr_pl, &constraint_ops);
...
struct rapl_package *rapl_find_package_domain(int cpu,
...
struct rapl_package *rapl_add_package(int cpu, struct rapl_if_priv *priv)
{
	...
	if (topology_max_die_per_package() > 1)
		snprintf(rp->name, PACKAGE_DOMAIN_NAME_LENGTH,
			 "package-%d-die-%d", topology_physical_package_id(cpu),
			 topology_die_id(cpu));
	else
...
static void power_limit_state_save(void)
{
	...
	list_for_each_entry (rp, &rapl_packages, plist) {
		...
		for (i = 0; i < nr_pl; i++) {
			...
			case PL1_ENABLE:
				ret = rapl_read_data_raw(
					rd, POWER_LIMIT1, true,
...
static int rapl_pm_callback(struct notifier_block *nb, unsigned long mode,
			    void *_unused)
{
	switch (mode) {
