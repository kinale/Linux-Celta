...
MODULE_PARM_DESC(
	ignore_sss,
	"Ignore staggered spinup flag (0=don't ignore, 1=ignore [default])");
...
static DEVICE_ATTR(em_buffer, S_IWUSR | S_IRUGO, ahci_read_em_buffer,
...
const struct attribute_group *ahci_shost_groups[] = { &ahci_shost_attr_group,
						      NULL };
EXPORT_SYMBOL_GPL(ahci_shost_groups);

static struct attribute *ahci_sdev_attrs[] = {
	&dev_attr_sw_activity.attr, &dev_attr_unload_heads.attr,
	&dev_attr_ncq_prio_supported.attr, &dev_attr_ncq_prio_enable.attr, NULL
};
...
const struct attribute_group *ahci_sdev_groups[] = { &ahci_sdev_attr_group,
						     NULL };
				    ...
				    const char *buf, size_t size)
{
	...
	if (!(ap->flags & ATA_FLAG_EM) ||
	    !(hpriv->em_msg_type & EM_MSG_TYPE_SGPIO) || size % 4 ||
	...
	for (i = 0; i < size; i += 4) {
		msg = msg_buf[i] | msg_buf[i + 1] << 8 | msg_buf[i + 2] << 16 |
		      msg_buf[i + 3] << 24;
...
void ahci_save_initial_config(struct device *dev, struct ahci_host_priv *hpriv)
{
	...
	if ((vers >> 16) > 1 || ((vers >> 16) == 1 && (vers & 0xFFFF) >= 0x200))
	...
	if ((cap2 & HOST_CAP2_SDS) && (hpriv->flags & AHCI_HFLAG_NO_DEVSLP)) {
		dev_info(dev, "controller can't do DEVSLP, turning off\n");
		cap2 &= ~HOST_CAP2_SDS;
		cap2 &= ~HOST_CAP2_SADM;
	}
...
static unsigned ahci_scr_offset(struct ata_port *ap, unsigned int sc_reg)
{
	static const int offset[] = {
		[SCR_STATUS] = PORT_SCR_STAT,	   [SCR_CONTROL] = PORT_SCR_CTL,
		[SCR_ERROR] = PORT_SCR_ERR,	   [SCR_ACTIVE] = PORT_SCR_ACT,
...
int ahci_stop_engine(struct ata_port *ap)
{
	...
	tmp = ata_wait_register(ap, port_mmio + PORT_CMD, PORT_CMD_LIST_ON,
			...
			unsigned int hints)
{
	...
	if ((hpriv->cap2 & HOST_CAP2_SDS) && (hpriv->cap2 & HOST_CAP2_SADM) &&
	...
	if (ap->flags & ATA_FLAG_EM) {
		ata_for_each_link (link, ap, EDGE) {
			...
			for (i = 0; i < EM_MAX_RETRY; i++) {
				rc = ap->ops->transmit_led_message(
					ap, emp->led_state, 4);
				/*
...
static ssize_t ahci_led_store(struct ata_port *ap, const char *buf, size_t size)
{
	unsigned int state;
	int pmp;
	struct ahci_port_priv *pp = ap->private_data;
...
static void ahci_port_init(struct device *dev, struct ata_port *ap, int port_no,
...
int ahci_kick_engine(struct ata_port *ap)
{
	...
	rc = 0;
	tmp = ata_wait_register(ap, port_mmio + PORT_CMD, PORT_CMD_CLO,
				PORT_CMD_CLO, 1, 500);
	if (tmp & PORT_CMD_CLO)
		rc = -EIO;
				...
				unsigned long timeout_msec)
{
	...
	if (timeout_msec) {
		tmp = ata_wait_register(ap, port_mmio + PORT_CMD_ISSUE, 0x1,
					0x1, 1, timeout_msec);
		if (tmp & 0x1) {
			ahci_kick_engine(ap);
			return -EBUSY;
				    ...
				    unsigned long deadline)
{
	...
	if (rc == -EIO) {
		...
		if (irq_sts & PORT_IRQ_BAD_PMP) {
			ata_link_warn(link, "applying PMP SRST workaround "
					    "and retrying\n");
...
static void ahci_error_intr(struct ata_port *ap, u32 irq_stat)
{
	...
	if (irq_stat & PORT_IRQ_UNK_FIS) {
		...
		ata_ehi_push_desc(active_ehi, "unknown FIS %08x %08x %08x %08x",
...
static void ahci_set_aggressive_devslp(struct ata_port *ap, bool sleep)
{
	...
	if (!sleep) {
		if (devslp & PORT_DEVSLP_ADSE) {
			...
			err_mask = ata_dev_set_feature(
				dev, SETFEATURES_SATA_DISABLE, SATA_DEVSLP);
	...
	devslp |= ((dito << PORT_DEVSLP_DITO_OFFSET) |
		   ...
		   (deto << PORT_DEVSLP_DETO_OFFSET) | PORT_DEVSLP_ADSE);
	...
	err_mask =
		ata_dev_set_feature(dev, SETFEATURES_SATA_ENABLE, SATA_DEVSLP);
	if (err_mask && err_mask != AC_ERR_DEV)
...
static int ahci_port_start(struct ata_port *ap)
{
	...
	if (ap->host->n_ports > 1) {
		...
		snprintf(pp->irq_desc, 8, "%s%d", dev_driver_string(dev),
			 ap->port_no);
	}
...
void ahci_print_info(struct ata_host *host, const char *scc_s)
{
	...
	dev_info(host->dev,
		 ...
		 (vers >> 24) & 0xff, (vers >> 16) & 0xff, (vers >> 8) & 0xff,
		 vers & 0xff,
