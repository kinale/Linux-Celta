...
static const struct scrubrate {
	...
	{ 0x01, 1600000000UL }, { 0x02, 800000000UL }, { 0x03, 400000000UL },
	{ 0x04, 200000000UL },	{ 0x05, 100000000UL }, { 0x06, 50000000UL },
	{ 0x07, 25000000UL },	{ 0x08, 12284069UL },  { 0x09, 6274509UL },
	{ 0x0A, 3121951UL },	{ 0x0B, 1560975UL },   { 0x0C, 781440UL },
	{ 0x0D, 390720UL },	{ 0x0E, 195300UL },    { 0x0F, 97650UL },
	{ 0x10, 48854UL },	{ 0x11, 24427UL },     { 0x12, 12213UL },
	{ 0x13, 6101UL },	{ 0x14, 3051UL },      { 0x15, 1523UL },
	{ 0x16, 761UL },	{ 0x00, 0UL }, /* scrubbing off */
};

int __amd64_read_pci_cfg_dword(struct pci_dev *pdev, int offset, u32 *val,
			       const char *func)
{
	int err = 0;
...
int __amd64_write_pci_cfg_dword(struct pci_dev *pdev, int offset, u32 val,
				const char *func)
{
	int err = 0;
						...
						u64 sys_addr)
{
	...
	if (unlikely((intlv_en != 0x01) && (intlv_en != 0x03) &&
		     (intlv_en != 0x07))) {
...
#define for_each_umc(i) for (i = 0; i < pvt->fam_type->max_mcs; i++)
			      ...
			      u64 *hole_offset, u64 *hole_size)
{
	...
	*hole_offset =
		(pvt->fam > 0xf) ? f10_dhar_offset(pvt) : k8_dhar_offset(pvt);
...
static struct attribute *dbg_attrs[] = {
	&dev_attr_dhar.attr,	&dev_attr_dbam.attr,	  &dev_attr_topmem.attr,
	&dev_attr_topmem2.attr, &dev_attr_dram_hole.attr, NULL
};
				  ...
				  const char *data, size_t count)
{
	...
	pr_notice_once(
		"Don't forget to decrease MCE polling interval in\n"
...
static struct attribute *inj_attrs[] = {
	&dev_attr_inject_section.attr,	  &dev_attr_inject_word.attr,
	&dev_attr_inject_ecc_vector.attr, &dev_attr_inject_write.attr,
	&dev_attr_inject_read.attr,	  NULL
};
...
static int df_indirect_read_instance(u16 node, u8 func, u16 reg, u8 instance_id,
				     u32 *lo)
{
...
{
	u8 hashed_bit = (ctx->ret_addr >> 12) ^ (ctx->ret_addr >> 18) ^
			(ctx->ret_addr >> 21) ^ (ctx->ret_addr >> 30) ^
...
static int get_intlv_mode_df2(struct addr_ctx *ctx)
{
	...
	if (ctx->intlv_mode != NONE && ctx->intlv_mode != NOHASH_2CH &&
	    ctx->intlv_mode != DF2_HASH_2CH)
		return -EINVAL;
...
static int get_cs_fabric_id_df2(struct addr_ctx *ctx)
{
	...
	if (df_indirect_read_instance(ctx->nid, 0, 0x50, ctx->inst_id,
				      &ctx->tmp))
		return -EINVAL;
...
static int get_masks_df2(struct addr_ctx *ctx)
{
	...
	if (df_indirect_read_broadcast(ctx->nid, 1, 0x208,
				       &ctx->reg_fab_id_mask0))
		return -EINVAL;
...
u8 umc_to_cs_mapping_aldebaran[] = { 28, 20, 24, 16, 12, 4,  8,	 0,  6,	 30, 2,
				     26, 22, 14, 18, 10, 19, 11, 15, 7,	 3,  27,
				     31, 23, 9,	 1,  5,	 29, 25, 17, 21, 13 };
...
static int get_dram_offset_reg(struct addr_ctx *ctx)
{
	...
	if (df_indirect_read_instance(ctx->nid, 0, 0x1B4, ctx->inst_id,
				      &ctx->reg_dram_offset))
		return -EINVAL;
...
static int calculate_cs_id(struct addr_ctx *ctx)
{
	...
	if (ctx->intlv_num_dies) {
		...
		ctx->cs_id |=
			calc_level_bits(ctx->cs_fabric_id, ctx->die_id_mask,
	...
	if (ctx->intlv_num_sockets) {
		...
		ctx->cs_id |=
			calc_level_bits(ctx->cs_fabric_id, ctx->socket_id_mask,
...
static int add_base_and_hole(struct addr_ctx *ctx)
{
	...
	if (ctx->reg_base_addr & BIT(1)) {
		...
		if (df_indirect_read_broadcast(ctx->nid, 0, 0x104,
					       &dram_hole_base))
			return -EINVAL;
...
{
	u64 dram_limit_addr =
		((ctx->reg_limit_addr & GENMASK_ULL(31, 12)) << 16) |
		GENMASK_ULL(27, 0);
...
{
	for (ctx->inst_id = 0; ctx->inst_id < ctx->num_blk_instances;
	     ctx->inst_id++) {
		/* Read D18F0x44 (FabricBlockInstanceInformation0). */
		if (df_indirect_read_instance(0, 0, 0x44, ctx->inst_id,
					      &ctx->tmp))
			return -EINVAL;
...
static int find_map_reg_by_dstfabricid(struct addr_ctx *ctx)
{
	...
	for (ctx->map_num = 0; ctx->map_num < DF_NUM_DRAM_MAPS_AVAILABLE;
	     ctx->map_num++) {
		...
		if ((dst_fabric_id & node_id_mask) ==
		    (ctx->cs_fabric_id & node_id_mask))
			return 0;
	}
...
static unsigned long determine_edac_cap(struct amd64_pvt *pvt)
{
	...
	if (pvt->umc) {
		...
		bit = (pvt->fam > 0xf || pvt->ext_model >= K8_REV_F) ? 19 : 17;
...
static int f17_get_cs_mode(int dimm, u8 ctrl, struct amd64_pvt *pvt)
{
     ...
     * be equal.
     */
	for_each_chip_select(base, ctrl, pvt) count +=
		csrow_enabled(base, ctrl, pvt);
...
static void __dump_misc_regs_df(struct amd64_pvt *pvt)
{
	...
	for_each_umc(i)
	{
		...
		amd_smn_read(pvt->mc_node_id, umc_base + UMCCH_ECC_BAD_SYMBOL,
			     &tmp);
		...
		edac_dbg(1, "UMC%d All DIMMs support ECC: %s\n", i,
			 (umc->umc_cfg & BIT(12)) ? "yes" : "no");
		edac_dbg(1, "UMC%d x4 DIMMs present: %s\n", i,
			 (umc->dimm_cfg & BIT(6)) ? "yes" : "no");
		edac_dbg(1, "UMC%d x16 DIMMs present: %s\n", i,
			 (umc->dimm_cfg & BIT(7)) ? "yes" : "no");

		if (pvt->dram_type == MEM_LRDDR4) {
			amd_smn_read(pvt->mc_node_id, umc_base + UMCCH_ADDR_CFG,
				     &tmp);
...
static void __dump_misc_regs(struct amd64_pvt *pvt)
{
	...
	edac_dbg(
		...
		pvt->dhar, dhar_base(pvt),
		(pvt->fam == 0xf) ? k8_dhar_offset(pvt) : f10_dhar_offset(pvt));
...
static void read_dram_base_limit_regs(struct amd64_pvt *pvt, unsigned range)
{
	...
	amd64_read_pci_cfg(pvt->F1, DRAM_BASE_LO + off,
			   &pvt->ranges[range].base.lo);
	amd64_read_pci_cfg(pvt->F1, DRAM_LIMIT_LO + off,
			   &pvt->ranges[range].lim.lo);
	...
	amd64_read_pci_cfg(pvt->F1, DRAM_BASE_HI + off,
			   &pvt->ranges[range].base.hi);
	amd64_read_pci_cfg(pvt->F1, DRAM_LIMIT_HI + off,
			   &pvt->ranges[range].lim.hi);
				    ...
				    struct err_info *err)
{
	...
	if (pvt->nbcfg & NBCFG_CHIPKILL) {
		err->channel =
			get_channel_from_ecc_syndrome(mci, err->syndrome);
		if (err->channel < 0) {
			/*
...
static int f17_early_channel_count(struct amd64_pvt *pvt)
{
	...
	for_each_umc(i) channels += !!(pvt->umc[i].sdp_ctrl & UMC_SDP_INIT);
...
static int gpu_early_channel_count(struct amd64_pvt *pvt)
{
	...
	for_each_umc(i) channels += pvt->csels[i].b_cnt;
				   ...
				   unsigned cs_mode, int cs_mask_nr)
{
	...
	if (cs_mode == 6 || cs_mode == 8 || cs_mode == 9 || cs_mode == 12)
		return -1;
	else
		return ddr3_cs_size(cs_mode, false);
}
				...
				bool hi_range_sel, u8 intlv_en)
{
	...
	if (dct_interleave_enabled(pvt)) {
		...
		if (intlv_addr & 0x2) {
			...
			u32 temp =
				hweight_long((u32)((sys_addr >> 16) & 0x1F)) &
				1;
...
static u64 f1x_get_norm_dct_addr(struct amd64_pvt *pvt, u8 range, u64 sys_addr,
				 bool hi_rng, u32 dct_sel_base_addr)
{
	u64 chan_off;
	...
	if (hi_rng) {
		...
		if ((!(dct_sel_base_addr >> 16) ||
		    ...
		    dhar_valid(pvt) && (sys_addr >= BIT_64(32)))
			chan_off = hole_off;
		else
	...
	return (sys_addr & GENMASK_ULL(47, 6)) -
	       (chan_off & GENMASK_ULL(47, 23));
}
...
static u64 f1x_swap_interleaved_region(struct amd64_pvt *pvt, u64 sys_addr)
{
	...
	if (!(sys_addr >> 34) &&
	    (((tmp_addr >= swap_base) && (tmp_addr <= swap_limit)) ||
	     (tmp_addr < rgn_size)))
				  ...
				  u64 sys_addr, int *chan_sel)
{
	...
	edac_dbg(1, "(range %d) SystemAddr= 0x%llx Limit=0x%llx\n", range,
		 sys_addr, get_dram_limit(pvt, range));

	if (dhar_valid(pvt) && dhar_base(pvt) <= sys_addr &&
	    sys_addr < BIT_64(32)) {
     ...
     */
	if (dct_high_range_enabled(pvt) && !dct_ganging_enabled(pvt) &&
	    ((sys_addr >> 27) >= (dct_sel_base >> 11)))
		high_range = true;

	channel = f1x_determine_channel(pvt, sys_addr, high_range, intlv_en);

	chan_addr = f1x_get_norm_dct_addr(pvt, range, sys_addr, high_range,
					  dct_sel_base);
	...
	if (dct_interleave_enabled(pvt) && !dct_high_range_enabled(pvt) &&
		...
		if (dct_sel_interleave_addr(pvt) != 1) {
			...
			/* A[12] */
			chan_addr =
				((chan_addr >> 13) << 12) | (chan_addr & 0xfff);
	}
				       ...
				       u64 sys_addr, int *chan_sel)
{
	...
	if (!(dct_cont_base_reg & BIT(0)) &&
	    !(dct_base <= (sys_addr >> 27) && dct_limit >= (sys_addr >> 27)))
		return -EINVAL;
	...
	if (num_dcts_intlv == 2) {
		if (intlv_addr == 0x4)
			chan_addr =
				((chan_addr >> 9) << 8) | (chan_addr & 0xff);
		else if (intlv_addr == 0x5)
			chan_addr =
				((chan_addr >> 10) << 9) | (chan_addr & 0x1ff);
		else
			return -EINVAL;
		...
		if (intlv_addr == 0x4)
			chan_addr =
				((chan_addr >> 10) << 8) | (chan_addr & 0xff);
		else if (intlv_addr == 0x5)
			chan_addr =
				((chan_addr >> 11) << 9) | (chan_addr & 0x1ff);
		else
			return -EINVAL;
	}
	...
	if (dct_offset_en) {
		amd64_read_pci_cfg(pvt->F1,
				   DRAM_CONT_HIGH_OFF + (int)channel * 4, &tmp);
				       ...
				       int *chan_sel)
{
	...
	for (range = 0; range < DRAM_RANGES; range++) {
		...
		if (pvt->fam == 0x15 && pvt->model >= 0x30)
			cs_found = f15_m30h_match_to_this_node(
				pvt, range, sys_addr, chan_sel);

		else if ((get_dram_base(pvt, range) <= sys_addr) &&
			 (get_dram_limit(pvt, range) >= sys_addr)) {
			cs_found = f1x_match_to_this_node(pvt, range, sys_addr,
							  chan_sel);
			if (cs_found >= 0)
				break;
		}
...
static void debug_display_dimm_sizes(struct amd64_pvt *pvt, u8 ctrl)
{
	...
	for (dimm = 0; dimm < 4; dimm++) {
		...
		if (dcsb[dimm * 2] & DCSB_CS_ENABLE)
             ...
             * corresponding DCSM.
             */
			size0 = pvt->ops->dbam_to_cs(
				pvt, ctrl, DBAM_DIMM(dimm, dbam), dimm);

		size1 = 0;
		if (dcsb[dimm * 2 + 1] & DCSB_CS_ENABLE)
			size1 = pvt->ops->dbam_to_cs(
				pvt, ctrl, DBAM_DIMM(dimm, dbam), dimm);
...
static const u16 x4_vectors[] = {
	0x2f57, 0x1afe, 0x66cc, 0xdd88, 0x11eb, 0x3396, 0x7f4c, 0xeac8, 0x0001,
	0x0002, 0x0004, 0x0008, 0x1013, 0x3032, 0x4044, 0x8088, 0x106b, 0x30d6,
	0x70fc, 0xe0a8, 0x4857, 0xc4fe, 0x13cc, 0x3288, 0x1ac5, 0x2f4a, 0x5394,
	0xa1e8, 0x1f39, 0x251e, 0xbd6c, 0x6bd8, 0x15c1, 0x2a42, 0x89ac, 0x4758,
	0x2b03, 0x1602, 0x4f0c, 0xca08, 0x1f07, 0x3a0e, 0x6b04, 0xbd08, 0x8ba7,
	0x465e, 0x244c, 0x1cc8, 0x2b87, 0x164e, 0x642c, 0xdc18, 0x40b9, 0x80de,
	0x1094, 0x20e8, 0x27db, 0x1eb6, 0x9dac, 0x7b58, 0x11c1, 0x2242, 0x84ac,
	0x4c58, 0x1be5, 0x2d7a, 0x5e34, 0xa718, 0x4b39, 0x8d1e, 0x14b4, 0x28d8,
	0x4c97, 0xc87e, 0x11fc, 0x33a8, 0x8e97, 0x497e, 0x2ffc, 0x1aa8, 0x16b3,
	0x3d62, 0x4f34, 0x8518, 0x1e2f, 0x391a, 0x5cac, 0xf858, 0x1d9f, 0x3b7a,
	0x572c, 0xfe18, 0x15f5, 0x2a5a, 0x5264, 0xa3b8, 0x1dbb, 0x3b66, 0x715c,
	0xe3f8, 0x4397, 0xc27e, 0x17fc, 0x3ea8, 0x1617, 0x3d3e, 0x6464, 0xb8b8,
	0x23ff, 0x12aa, 0xab6c, 0x56d8, 0x2dfb, 0x1ba6, 0x913c, 0x7328, 0x185d,
	0x2ca6, 0x7914, 0x9e28, 0x171b, 0x3e36, 0x7d7c, 0xebe8, 0x4199, 0x82ee,
	0x19f4, 0x2e58, 0x4807, 0xc40e, 0x130c, 0x3208, 0x1905, 0x2e0a, 0x5804,
	0xac08, 0x213f, 0x132a, 0xadfc, 0x5ba8, 0x19a9, 0x2efe, 0xb5cc, 0x6f88,
};

static const u16 x8_vectors[] = {
	0x0145, 0x028a, 0x2374, 0x43c8, 0xa1f0, 0x0520, 0x0a40, 0x1480, 0x0211,
	0x0422, 0x0844, 0x1088, 0x01b0, 0x44e0, 0x23c0, 0xed80, 0x1011, 0x0116,
	0x022c, 0x0458, 0x08b0, 0x8c60, 0x2740, 0x4e80, 0x0411, 0x0822, 0x1044,
	0x0158, 0x02b0, 0x2360, 0x46c0, 0xab80, 0x0811, 0x1022, 0x012c, 0x0258,
	0x04b0, 0x4660, 0x8cc0, 0x2780, 0x2071, 0x40e2, 0xa0c4, 0x0108, 0x0210,
	0x0420, 0x0840, 0x1080, 0x4071, 0x80e2, 0x0104, 0x0208, 0x0410, 0x0820,
	0x1040, 0x2080, 0x8071, 0x0102, 0x0204, 0x0408, 0x0810, 0x1020, 0x2040,
	0x4080, 0x019d, 0x03d6, 0x136c, 0x2198, 0x50b0, 0xb2e0, 0x0740, 0x0e80,
	0x0189, 0x03ea, 0x072c, 0x0e58, 0x1cb0, 0x56e0, 0x37c0, 0xf580, 0x01fd,
	0x0376, 0x06ec, 0x0bb8, 0x1110, 0x2220, 0x4440, 0x8880, 0x0163, 0x02c6,
	0x1104, 0x0758, 0x0eb0, 0x2be0, 0x6140, 0xc280, 0x02fd, 0x01c6, 0x0b5c,
	0x1108, 0x07b0, 0x25a0, 0x8840, 0x6180, 0x0801, 0x012e, 0x025c, 0x04b8,
	0x1370, 0x26e0, 0x57c0, 0xb580, 0x0401, 0x0802, 0x015c, 0x02b8, 0x22b0,
	0x13e0, 0x7140, 0xe280, 0x0201, 0x0402, 0x0804, 0x01b8, 0x11b0, 0x31a0,
	0x8040, 0x7180, 0x0101, 0x0202, 0x0404, 0x0808, 0x1010, 0x2020, 0x4040,
	0x8080, 0x0001, 0x0002, 0x0004, 0x0008, 0x0010, 0x0020, 0x0040, 0x0080,
			    ...
			    u8 ecc_type)
{
	...
	edac_mc_handle_error(err_type, mci, 1, err->page, err->offset,
			     err->syndrome, err->csrow, err->channel, -1,
			     string, "");
}
...
static void decode_umc_error(int node_id, struct mce *m)
{
     ...
     */
	if (pvt->is_gpu)
		df_inst_id = (err.csrow * pvt->channel_count / mci->nr_csrows) +
			     err.channel;
	else
				   ...
				   u16 pci_id2)
{
	if (pvt->umc) {
		...
		pvt->F6 = pci_get_related_function(pvt->F3->vendor, pci_id2,
						   pvt->F3);
		if (!pvt->F6) {
			pci_dev_put(pvt->F0);
			pvt->F0 = NULL;
...
static void __read_mc_regs_df(struct amd64_pvt *pvt)
{
	...
	for_each_umc(i)
	{
		...
		amd_smn_read(nid, umc_base + UMCCH_UMC_CAP_HI,
			     &umc->umc_cap_hi);
	}
}
...
static void read_mc_regs(struct amd64_pvt *pvt)
{
	...
	for (range = 0; range < DRAM_RANGES; range++) {
		...
		edac_dbg(
			...
			(rw & 0x1) ? "R" : "-", (rw & 0x2) ? "W" : "-",
			dram_intlv_sel(pvt, range), dram_dst_node(pvt, range));
	}
...
static int init_csrows(struct mem_ctl_info *mci)
{
	...
	edac_dbg(0, "node %d, NBCFG=0x%08x[ChipKillEccCap: %d|DramEccEn: %d]\n",
		 pvt->mc_node_id, val, !!(val & NBCFG_CHIPKILL),
		 !!(val & NBCFG_ECC_ENABLE));
...
static inline void f17h_determine_edac_ctl_cap(struct mem_ctl_info *mci,
					       struct amd64_pvt *pvt)
{
...
static int hw_info_get(struct amd64_pvt *pvt)
{
	...
	if (pvt->fam >= 0x17) {
		pvt->umc = kcalloc(pvt->fam_type->max_mcs,
...
static int init_one_instance(struct amd64_pvt *pvt)
{
	...
	layers[0].size =
		pvt->is_gpu ? pvt->fam_type->max_mcs : pvt->csels[0].b_cnt;
     ...
     */
	layers[1].size =
		pvt->is_gpu ? pvt->csels[0].b_cnt : pvt->fam_type->max_mcs;
...
static bool instance_has_memory(struct amd64_pvt *pvt)
{
	...
	for (dct = 0; dct < pvt->fam_type->max_mcs; dct++) {
		for_each_chip_select(cs, dct, pvt) cs_enabled |=
			csrow_enabled(cs, dct, pvt);
	}
...
MODULE_DESCRIPTION(
	"MC support for AMD64 memory controllers - " EDAC_AMD64_VERSION);
