...
static const char *const rrrr_msgs[] = { "GEN", "RD",  "WR", "DRD", "DWR",
					 "IRD", "PRF", "EV", "SNP" };
...
static const char *const mc5_mce_desc[] = { "CPU Watchdog timer expire",
...
static const char *const mc6_mce_desc[] = {
	"Hardware Assertion", "Free List",	 "Physical Register File",
	"Retire Queue",	      "Scheduler table", "Status Register File",
};
...
static const char *const smca_pie_mce_desc[] = {
	"Hardware Assert", "Register security violation", "Link Error",
	"Poison data consumption", "A deferred error was detected in the DF"
};
...
static const char *const smca_nbio_mce_desc[] = {
	"ECC or Parity error",	      "PCIE error",
	"SDP ErrEvent error",	      "SDP Egress Poison Error",
...
static struct smca_mce_desc smca_mce_descs[] = {
	...
	[SMCA_PCIE_V2] = { smca_pcie2_mce_desc,
			   ARRAY_SIZE(smca_pcie2_mce_desc) },
	[SMCA_XGMI_PCS] = { smca_xgmipcs_mce_desc,
			    ARRAY_SIZE(smca_xgmipcs_mce_desc) },
	[SMCA_XGMI_PHY] = { smca_xgmiphy_mce_desc,
			    ARRAY_SIZE(smca_xgmiphy_mce_desc) },
	[SMCA_WAFL_PHY] = { smca_waflphy_mce_desc,
			    ARRAY_SIZE(smca_waflphy_mce_desc) },
};
...
static void decode_mc1_mce(struct mce *m)
{
	...
	if (TLB_ERROR(ec))
		...
		bool k8 =
			(boot_cpu_data.x86 == 0xf && (m->status & BIT_64(58)));
			  ...
			  void *data)
{
	...
	pr_emerg(HW_ERR "CPU:%d (%x:%x:%x) MC%d_STATUS[%s|%s|%s|%s|%s",
		 m->extcpu, fam, x86_model(m->cpuid), x86_stepping(m->cpuid),
		 m->bank, ((m->status & MCI_STATUS_OVER) ? "Over" : "-"),
	...
	if (boot_cpu_has(X86_FEATURE_SMCA)) {
		...
		if (!rdmsr_safe(addr, &low, &high) && (low & MCI_CONFIG_MCAX))
			pr_cont("|%s",
				((m->status & MCI_STATUS_TCC) ? "TCC" : "-"));

		pr_cont("|%s",
			((m->status & MCI_STATUS_SYNDV) ? "SyndV" : "-"));
	}
