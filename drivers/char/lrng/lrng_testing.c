				      ...
				      u8 *outbuf, u32 outbuflen)
{
	...
	while (outbuflen) {
		...
		if (!data->rb_writer || (data->rb_writer == data->rb_reader)) {
			spin_unlock_irqrestore(&data->lock, flags);
...
MODULE_PARM_DESC(
	...
	"Enable gathering boot time entropy of the first IRQ flags entropy events");
...
{
	return lrng_testing_reader(&lrng_raw_regs, &boot_raw_regs_test, outbuf,
				   outbuflen);
}
...
MODULE_PARM_DESC(
	boot_irq_perf,
	"Enable gathering boot time interrupt performance data of the first entropy events");
 ...
 */
static u8
	lrng_acvt_hash_data[LRNG_ACVT_MAX_SHA_MSG] __aligned(LRNG_KCAPI_ALIGN);
				   ...
				   size_t count, loff_t *ppos)
{
	...
	ret = crypto_cb->lrng_hash_init(shash, NULL) ?:
		      crypto_cb->lrng_hash_update(shash, lrng_acvt_hash_data, atomic_read_u32(&lrng_acvt_hash_data_size)) ?:
...
#endif
#ifdef CONFIG_LRNG_RAW_REGS_ENTROPY
	debugfs_create_file_unsafe("lrng_raw_regs", 0400, lrng_raw_debugfs_root,
				   NULL, &lrng_raw_regs_fops);
#endif
#ifdef CONFIG_LRNG_RAW_ARRAY
