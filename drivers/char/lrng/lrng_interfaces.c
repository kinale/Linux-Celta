...
void lrng_debug_report_seedlevel(const char *name)
{
	...
	if (!lrng_state_min_seeded())
		pr_notice(
			"%pS %s called without reaching minimally seeded level (available entropy %u)\n",
...
{
	lrng_pool_insert_aux(
		buf, size,
		IS_ENABLED(CONFIG_RANDOM_TRUST_BOOTLOADER) ? size * 8 : 0);
}
...
static ssize_t lrng_read_common_block(int nonblock, char __user *buf,
				      size_t nbytes)
{
	if (nbytes == 0)
		return 0;
			      ...
			      size_t nbytes, loff_t *ppos)
{
	if (!lrng_state_min_seeded())
		pr_notice_ratelimited(
			"%s - use of insufficiently seeded DRNG (%zu bytes read)\n",
			current->comm, nbytes);
	else if (!lrng_state_operational())
		pr_debug_ratelimited(
			"%s - use of not fully seeded DRNG (%zu bytes read)\n",
			current->comm, nbytes);
