...
static int lrng_pool_insert_aux_locked(const u8 *inbuf, u32 inbuflen,
				       u32 entropy_bits)
{
	struct lrng_pool *pool = &lrng_pool;
	struct shash_desc *shash = (struct shash_desc *)pool->aux_pool;
...
static inline u32 lrng_get_aux_pool(u8 *outbuf, u32 requested_bits)
{
	...
	unsigned long flags;
	void *hash;
	u32 collected_ent_bits, returned_ent_bits, unused_bits = 0, digestsize;
