...
static inline void lrng_data_process_selftest_insert(u32 time)
{
	...
	lrng_data_selftest[array] &= ~(
		lrng_data_slot_val(0xffffffff & LRNG_DATA_SLOTSIZE_MASK, slot));
	lrng_data_selftest[array] |=
...
static unsigned int lrng_chacha20_drng_selftest(void)
{
	...
	ret = crypto_cb->lrng_drng_seed_helper(
		&chacha20, seed, sizeof(expected_block_nonalinged));
	if (ret < 0)
		goto err;
	ret = crypto_cb->lrng_drng_generate_helper(
		&chacha20, outbuf, sizeof(expected_block_nonalinged));
