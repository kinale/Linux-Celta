...
u32 lrng_pcpu_avail_entropy(void)
{
	...
	if (!lrng_pcpu_continuous_compression) {
		...
		digestsize_irqs =
			min_t(u32, digestsize_irqs, LRNG_DATA_NUM_VALUES);
	}
 ...
 */
int lrng_pcpu_switch_hash(int node, const struct lrng_crypto_cb *new_cb,
			  void *new_hash, const struct lrng_crypto_cb *old_cb)
{
	...
	for_each_online_cpu (cpu) {
		...
		atomic_add_return_relaxed(
			found_irqs, per_cpu_ptr(&lrng_pcpu_array_irqs, cpu));

		pr_debug(
			"Re-initialize per-CPU entropy pool for CPU %d on NUMA node %d with hash %s\n",
			...
			void *pcpu_hash, int cpu, u8 *digest, u32 *digestsize)
{
	...
	*digestsize = pcpu_crypto_cb->lrng_hash_digestsize(pcpu_hash);
	...
	found_irqs =
		atomic_xchg_relaxed(per_cpu_ptr(&lrng_pcpu_array_irqs, cpu), 0);
	...
	if (pcpu_crypto_cb->lrng_hash_update(pcpu_shash, (u8 *)per_cpu_ptr(lrng_pcpu_array, cpu), LRNG_DATA_ARRAY_SIZE * sizeof(u32)) ?:
		    ...
		    pcpu_crypto_cb->lrng_hash_init(pcpu_shash, pcpu_hash) ?:
			  ...
			  pcpu_crypto_cb->lrng_hash_update(pcpu_shash, digest,
						     *digestsize))
		found_irqs = 0;
...
u32 lrng_pcpu_pool_hash(u8 *outbuf, u32 requested_bits, bool fully_seeded)
{
	...
	requested_irqs =
		lrng_entropy_to_data(requested_bits + lrng_compress_osr());
	...
	for_each_online_cpu (cpu) {
		...
		if (pcpu_drng == drng) {
			found_irqs = lrng_pcpu_pool_hash_one(
				crypto_cb, hash, cpu, digest, &digestsize);
		} else {
...
static inline void lrng_pcpu_array_compress(void)
{
	...
	if (unlikely(init) && crypto_cb->lrng_hash_init(shash, hash)) {
		...
		if (crypto_cb->lrng_hash_update(
			    shash, (u8 *)this_cpu_ptr(lrng_pcpu_array),
...
static inline void _lrng_pcpu_array_add_u32(u32 data)
{
	...
	u32 pre_ptr, mask,
		ptr = this_cpu_add_return(lrng_pcpu_array_ptr,
	...
	this_cpu_write(lrng_pcpu_array[lrng_data_idx2array(ptr)], data & mask);
...
static inline void lrng_pcpu_array_add_slot(u32 data)
{
	/* Get slot */
	u32 ptr =
		this_cpu_inc_return(lrng_pcpu_array_ptr) & LRNG_DATA_WORD_MASK;
	...
	BUILD_BUG_ON(LRNG_DATA_ARRAY_MEMBER_BITS != sizeof(lrng_pcpu_array[0])
							    << 3);
...
static inline void lrng_time_process_common(u32 time,
					    void (*add_time)(u32 data))
{
