...
static void lrng_drng_inject(struct lrng_drng *drng, const u8 *inbuf,
			     u32 inbuflen, bool fully_seeded)
{
	const char *drng_type =
		unlikely(drng == &lrng_drng_atomic) ? "atomic" : "regular";
	unsigned long flags = 0;

	BUILD_BUG_ON(LRNG_DRNG_RESEED_THRESH > INT_MAX);
	pr_debug("seeding %s DRNG with %u bytes\n", drng_type, inbuflen);
	lrng_drng_lock(drng, &flags);
	if (drng->crypto_cb->lrng_drng_seed_helper(drng->drng, inbuf,
						   inbuflen) < 0) {
...
static void lrng_drng_seed(struct lrng_drng *drng)
{
	...
	if ((drng->drng != lrng_drng_atomic.drng) &&
		...
		u8 seedbuf[LRNG_DRNG_SECURITY_STRENGTH_BYTES] __aligned(
			LRNG_KCAPI_ALIGN);
...
{
	pr_debug(
		"reseed triggered by interrupt noise source for DRNG on NUMA node %d\n",
		node);
	lrng_drng_seed(drng);
	if (drng->fully_seeded) {
