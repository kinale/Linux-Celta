...
int __init rand_initialize(void)
{
	struct seed {
		ktime_t time;
		unsigned long
			data[(LRNG_MAX_DIGESTSIZE / sizeof(unsigned long))];
...
{
	struct lrng_state *state = &lrng_state;
	u32 req_ent = lrng_sp80090c_compliant() ? lrng_security_strength() :
	...
	entropy_buf->b_bits = lrng_pcpu_pool_hash(
		entropy_buf->b, requested_bits, state->lrng_fully_seeded);
