				  ...
				  const char **endptr)
{
	...
	*endptr = strchrnul(path, ';');
	...
	ret = sscanf(wpath, "%x:%x:%x.%x%c", &seg, &bus, &slot, &func, &end);
	if (ret != 4) {
		seg = 0;
	...
	ret = (seg == pci_domain_nr(dev->bus) && bus == dev->bus->number &&
...
static u8 __pci_find_next_cap(struct pci_bus *bus, unsigned int devfn, u8 pos,
			      int cap)
{
...
static u8 __pci_bus_find_cap_start(struct pci_bus *bus, unsigned int devfn,
				   u8 hdr_type)
{
	u16 status;
...
static u8 __pci_find_next_ht_cap(struct pci_dev *dev, u8 pos, int ht_cap)
{
	...
	pos = __pci_find_next_cap_ttl(dev->bus, dev->devfn, pos, PCI_CAP_ID_HT,
				      &ttl);
	while (pos) {
...
static void pci_disable_acs_redir(struct pci_dev *dev)
{
	...
	while (*p) {
		...
		if (ret < 0) {
			pr_info_once(
				"PCI: Can't parse disable_acs_redir parameter: %s\n",
...
int pci_set_power_state(struct pci_dev *dev, pci_power_t state)
{
     ...
     */
	error = pci_raw_set_power_state(dev,
					state > PCI_D3hot ? PCI_D3hot : state);
...
static int pci_save_pcix_state(struct pci_dev *dev)
{
	...
	pci_read_config_word(dev, pos + PCI_X_CMD, (u16 *)save_state->cap.data);
...
static void pci_restore_config_space_range(struct pci_dev *pdev, int start,
					   int end, int retry, bool force)
{
	int index;
	...
	if (dev->pme_poll) {
		...
		if (enable) {
			...
			if (list_is_singular(&pci_pme_list))
				queue_delayed_work(
					system_freezable_wq, &pci_pme_work,
					msecs_to_jiffies(PME_TIMEOUT));
			mutex_unlock(&pci_pme_list_mutex);
		} else {
			mutex_lock(&pci_pme_list_mutex);
 ...
 */
static int _pci_add_cap_save_buffer(struct pci_dev *dev, u16 cap, bool extended,
				    unsigned int size)
{
	int pos;
	struct pci_cap_saved_state *save_state;
...
{
	return pci_request_selected_regions(pdev, ((1 << PCI_STD_NUM_BARS) - 1),
					    res_name);
}
...
int pci_request_regions_exclusive(struct pci_dev *pdev, const char *res_name)
{
	return pci_request_selected_regions_exclusive(
		pdev, ((1 << PCI_STD_NUM_BARS) - 1), res_name);
}
...
unlock:
	list_for_each_entry_continue_reverse (dev, &slot->bus->devices,
					      bus_list) {
		if (!dev->slot || dev->slot != slot)
			continue;
		if (dev->subordinate)
				  ...
				  unsigned int command_bits, u32 flags)
{
	if (arch_set_vga_state)
		return arch_set_vga_state(dev, decode, command_bits, flags);
	return 0;
}
...
bool pci_devs_are_dma_aliases(struct pci_dev *dev1, struct pci_dev *dev2)
{
	return (dev1->dma_alias_mask &&
		...
		test_bit(dev1->devfn, dev2->dma_alias_mask)) ||
	       pci_real_dma_dev(dev1) == dev2 || pci_real_dma_dev(dev2) == dev1;
}
							...
							bool *resize)
{
	...
	if (pci_has_flag(PCI_PROBE_ONLY)) {
		align = 0;
		pr_info_once(
			"PCI: Ignoring requested alignments (PCI_PROBE_ONLY)\n");
		goto out;
	}
...
static ssize_t resource_alignment_store(struct bus_type *bus, const char *buf,
					size_t count)
{
	char *param, *old, *end;
...
{
	return bus_create_file(&pci_bus_type, &bus_attr_resource_alignment);
}
...
{
	resource_alignment_param =
		kstrdup(resource_alignment_param, GFP_KERNEL);
