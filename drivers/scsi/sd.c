...
static const char *sd_cache_types[] = { "write through", "none", "write back",
				...
				size_t count)
{
	...
	if (scsi_mode_sense(sdp, 0x08, 8, buffer, sizeof(buffer), SD_TIMEOUT,
		...
		return -EINVAL;
	len = min_t(size_t, sizeof(buffer),
		    data.length - data.header_length -
			    data.block_descriptor_length);
	buffer_data =
		buffer + data.header_length + data.block_descriptor_length;
	buffer_data[2] &= ~0x05;
...
static ssize_t manage_start_stop_store(struct device *dev,
...
static const char *lbp_mode[] = {
	[SD_LBP_FULL] = "full",		  [SD_LBP_UNMAP] = "unmap",
	[SD_LBP_WS16] = "writesame_16",	  [SD_LBP_WS10] = "writesame_10",
	[SD_LBP_ZERO] = "writesame_zero", [SD_LBP_DISABLE] = "disabled",
};
			 ...
			 size_t len, bool send)
{
	...
	ret = scsi_execute(sdev, cdb, send ? DMA_TO_DEVICE : DMA_FROM_DEVICE,
			   buffer, len, NULL, NULL, SD_TIMEOUT,
...
static unsigned int sd_prot_flag_mask(unsigned int prot_op)
{
	static const unsigned int flag_mask[] = {
		...
		[SCSI_PROT_READ_STRIP] =
			SCSI_PROT_TRANSFER_PI | SCSI_PROT_GUARD_CHECK |
			SCSI_PROT_REF_CHECK | SCSI_PROT_REF_INCREMENT,

		[SCSI_PROT_READ_INSERT] =
			SCSI_PROT_REF_INCREMENT | SCSI_PROT_IP_CHECKSUM,
		...
		[SCSI_PROT_WRITE_INSERT] =
			SCSI_PROT_TRANSFER_PI | SCSI_PROT_REF_INCREMENT,

		[SCSI_PROT_WRITE_STRIP] =
			SCSI_PROT_GUARD_CHECK | SCSI_PROT_REF_CHECK |
			SCSI_PROT_REF_INCREMENT | SCSI_PROT_IP_CHECKSUM,
...
static blk_status_t sd_setup_read_write_cmnd(struct scsi_cmnd *cmd)
{
	...
	SCSI_LOG_HLQUEUE(1, scmd_printk(KERN_INFO, cmd,
					"%s: block=%llu, count=%d\n", __func__,
					(unsigned long long)blk_rq_pos(rq),
					blk_rq_sectors(rq)));
	SCSI_LOG_HLQUEUE(2, scmd_printk(KERN_INFO, cmd,
					"%s %d/%u 512 byte blocks.\n",
					write ? "writing" : "reading",
					nr_blocks, blk_rq_sectors(rq)));
 ...
 **/
static int sd_ioctl(struct block_device *bdev, fmode_t mode, unsigned int cmd,
		    unsigned long arg)
{
...
static int sd_sync_cache(struct scsi_disk *sdkp, struct scsi_sense_hdr *sshdr)
{
	...
	struct scsi_device *sdp = sdkp->device;
	const int timeout =
		sdp->request_queue->rq_timeout * SD_FLUSH_TIMEOUT_MULTIPLIER;
...
static int sd_pr_command(struct block_device *bdev, u8 sa, u64 key, u64 sa_key,
			 u8 type, u8 flags)
{
	struct scsi_disk *sdkp = scsi_disk(bdev->bd_disk);
	struct scsi_device *sdev = sdkp->device;
...
static int sd_eh_action(struct scsi_cmnd *scmd, int eh_disp)
{
	...
	if (!scsi_device_online(sdev) || !scsi_medium_access_command(scmd) ||
	    host_byte(scmd->result) != DID_TIME_OUT || eh_disp != SUCCESS)
		return eh_disp;
...
static unsigned int sd_completed_bytes(struct scsi_cmnd *scmd)
{
	...
	if (!scsi_get_sense_info_fld(scmd->sense_buffer, SCSI_SENSE_BUFFERSIZE,
				     &bad_lba))
		return 0;
				...
				int the_result)
{
     ...
     */
	if (sdp->removable && sense_valid && sshdr->sense_key == NOT_READY)
		set_media_not_present(sdkp);
 ...
 * Print disk capacity
 */
static void sd_print_capacity(struct scsi_disk *sdkp, sector_t old_capacity)
{
	...
	string_get_size(sdkp->capacity, sector_size, STRING_UNITS_2, cap_str_2,
			sizeof(cap_str_2));
	string_get_size(sdkp->capacity, sector_size, STRING_UNITS_10,
			cap_str_10, sizeof(cap_str_10));

	sd_printk(KERN_NOTICE, sdkp, "%llu %d-byte logical blocks: (%s/%s)\n",
		  (unsigned long long)sdkp->capacity, sector_size, cap_str_10,
		  cap_str_2);
				       ...
				       unsigned char *buffer)
{
	...
	if (sdp->use_192_bytes_for_3f) {
         ...
         */
		if (res < 0)
			res = sd_do_mode_sense(sdkp, 0, 0, buffer, 4, &data,
					       NULL);
...
static void sd_read_cache_type(struct scsi_disk *sdkp, unsigned char *buffer)
{
	...
	/* cautiously ask */
	res = sd_do_mode_sense(sdkp, dbd, modepage, buffer, first_len, &data,
			       &sshdr);
	...
	/* Get the data */
	if (len > first_len)
		res = sd_do_mode_sense(sdkp, dbd, modepage, buffer, len, &data,
				       &sshdr);
...
static void sd_read_app_tag_own(struct scsi_disk *sdkp, unsigned char *buffer)
{
	...
	if (res < 0 || !data.header_length || data.length < 6) {
		sd_first_printk(
			KERN_WARNING, sdkp,
...
static int sd_probe(struct device *dev)
{
	...
	error = -ENODEV;
	if (sdp->type != TYPE_DISK && sdp->type != TYPE_ZBC &&
	    sdp->type != TYPE_MOD && sdp->type != TYPE_RBC)
		goto out;
	...
	SCSI_LOG_HLQUEUE(3, sdev_printk(KERN_INFO, sdp, "sd_probe\n"));
	...
	if (sdp->rpm_autosuspend) {
		pm_runtime_set_autosuspend_delay(
			dev, sdp->host->hostt->rpm_autosuspend_delay);
	}
...
static int sd_resume_runtime(struct device *dev)
{
	...
	if (sdp->ignore_media_change) {
		...
		if (scsi_execute(sdp, cmd, DMA_NONE, NULL, 0, NULL, NULL,
				 sdp->request_queue->rq_timeout, 1, 0, RQF_PM,
				 NULL))
			sd_printk(KERN_NOTICE, sdkp,
...
static int __init init_sd(void)
{
	...
	sd_cdb_cache =
		kmem_cache_create("sd_ext_cdb", SD_EXT_CDB_SIZE, 0, 0, NULL);
	if (!sd_cdb_cache) {
...
void sd_print_result(const struct scsi_disk *sdkp, const char *msg, int result)
{
	...
	if (hb_string)
		sd_printk(KERN_INFO, sdkp,
			  ...
			  hb_string ? hb_string : "invalid", "DRIVER_OK");
	else
		sd_printk(KERN_INFO, sdkp,
			  "%s: Result: hostbyte=0x%02x driverbyte=%s\n", msg,
			  host_byte(result), "DRIVER_OK");
}
