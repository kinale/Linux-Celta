...
static inline bool page_range_in_range(struct ashmem_range *range, size_t start,
				       size_t end)
{
...
static unsigned long ashmem_vmfile_get_unmapped_area(struct file *file,
						     unsigned long addr,
						     unsigned long len,
						     unsigned long pgoff,
						     unsigned long flags)
		      ...
		      struct ashmem_range **new_range)
{
	...
	list_for_each_entry_safe (range, next, &asma->unpinned_list, unpinned) {
		...
		if (page_range_in_range(range, pgstart, pgend)) {
             ...
             */
			range_alloc(asma, range, range->purged, pgend + 1,
				    range->pgend, new_range);
...
static int __init ashmem_init(void)
{
	...
	ashmem_area_cachep = kmem_cache_create(
		"ashmem_area_cache", sizeof(struct ashmem_area), 0, 0, NULL);
