...
xlog_find_buffer_cancelled(struct xlog *log, xfs_daddr_t blkno, uint len)
{
...
static bool xlog_add_buffer_cancelled(struct xlog *log, xfs_daddr_t blkno,
				      uint len)
{
	struct xfs_buf_cancel *bcp;
 ...
 */
static bool xlog_put_buffer_cancelled(struct xlog *log, xfs_daddr_t blkno,
				      uint len)
{
	struct xfs_buf_cancel *bcp;
...
xlog_recover_buf_reorder(struct xlog_recover_item *item)
{
...
STATIC void xlog_recover_buf_ra_pass2(struct xlog *log,
 ...
 */
static int xlog_recover_buf_commit_pass1(struct xlog *log,
					 struct xlog_recover_item *item)
{
 ...
 */
static void xlog_recover_validate_buf_type(struct xfs_mount *mp,
					   struct xfs_buf *bp,
				       ...
				       xfs_lsn_t current_lsn)
{
	...
	while (1) {
		...
		fa = NULL;
		if (buf_f->blf_flags &
		    (XFS_BLF_UDQUOT_BUF | XFS_BLF_PDQUOT_BUF |
		     XFS_BLF_GDQUOT_BUF)) {
			if (item->ri_buf[i].i_addr == NULL) {
				xfs_alert(mp, "XFS: NULL dquot in %s.",
					  __func__);
				goto next;
			}
 ...
 */
STATIC bool xlog_recover_do_dquot_buffer(struct xfs_mount *mp, struct xlog *log,
					 struct xlog_recover_item *item,
					 struct xfs_buf *bp,
					 struct xfs_buf_log_format *buf_f)
{
					...
					struct xfs_buf_log_format *buf_f)
{
	...
	for (i = 0; i < inodes_per_buf; i++) {
		...
		if (XFS_IS_CORRUPT(mp, *logged_nextp == 0)) {
			xfs_alert(
				mp,
				"Bad inode buffer log record (ptr = " PTR_FMT
				", bp = " PTR_FMT "). "
		...
		*buffer_nextp = *logged_nextp;
         ...
         */
		xfs_dinode_calc_crc(
			mp, xfs_buf_offset(bp, i * mp->m_sb.sb_inodesize));
	}
 ...
 * over the top of.
 */
static xfs_lsn_t xlog_recover_get_buf_lsn(struct xfs_mount *mp,
					  struct xfs_buf *bp,
					  struct xfs_buf_log_format *buf_f)
{
	uint32_t magic32;
 ...
 */
STATIC int xlog_recover_buf_commit_pass2(struct xlog *log,
					 ...
					 xfs_lsn_t current_lsn)
{
	...
	if (buf_f->blf_flags & XFS_BLF_INODE_BUF) {
		...
		if (error)
			goto out_release;
	} else if (buf_f->blf_flags & (XFS_BLF_UDQUOT_BUF | XFS_BLF_PDQUOT_BUF |
				       XFS_BLF_GDQUOT_BUF)) {
		bool dirty;
