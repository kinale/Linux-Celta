...
static struct iomap_page *iomap_page_create(struct inode *inode,
					    struct page *page)
{
...
static void iomap_page_release(struct page *page)
{
 ...
 */
static void iomap_adjust_read_range(struct inode *inode, struct iomap_page *iop,
				    loff_t *pos, loff_t length, unsigned *offp,
				    unsigned *lenp)
{
	loff_t orig_pos = *pos;
...
static void iomap_iop_set_range_uptodate(struct page *page, unsigned off,
					 unsigned len)
{
	struct iomap_page *iop = to_iomap_page(page);
...
static void iomap_read_page_end_io(struct bio_vec *bvec, int error)
{
...
static int iomap_read_page_sync(loff_t block_start, struct page *page,
...
static void iomap_finish_page_writeback(struct inode *inode, struct page *page,
 ...
 * ioend after this.
 */
static void iomap_finish_ioend(struct iomap_ioend *ioend, int error)
{
 ...
 */
static void iomap_add_to_ioend(struct inode *inode, loff_t offset,
 ...
 */
static int iomap_do_writepage(struct page *page, struct writeback_control *wbc,
			      void *data)
{
