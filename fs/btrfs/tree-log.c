...
static int btrfs_log_inode(struct btrfs_trans_handle *trans,
			   struct btrfs_inode *inode, int inode_only,
			   ...
			   struct btrfs_root *root, struct btrfs_log_ctx *ctx)
{
	...
	if (!test_bit(BTRFS_ROOT_HAS_LOG_TREE, &tree_root->state)) {
		...
		if (!fs_info->log_root_tree) {
			...
			if (!ret) {
				set_bit(BTRFS_ROOT_HAS_LOG_TREE,
					&tree_root->state);
				created = true;
			}
		}
...
{
	filemap_fdatawait_range(buf->pages[0]->mapping, buf->start,
				buf->start + buf->len - 1);
}
 ...
 */
static int process_one_buffer(struct btrfs_root *log, struct extent_buffer *eb,
			      struct walk_control *wc, u64 gen, int level)
{
...
insert:
	...
	ret = btrfs_insert_empty_item(trans, root, path, key, item_size);
	path->skip_release_on_error = 0;

	/* make sure any existing item is the correct size */
	if (ret == -EEXIST || ret == -EOVERFLOW) {
		u32 found_size;
		found_size = btrfs_item_size_nr(path->nodes[0], path->slots[0]);
	...
	if (key->type == BTRFS_INODE_ITEM_KEY && ret == -EEXIST) {
		...
		if (overwrite_root && S_ISDIR(btrfs_inode_mode(eb, src_item)) &&
		    S_ISDIR(btrfs_inode_mode(path->nodes[0], dst_item))) {
			save_old_i_size = 1;
			saved_i_size =
				btrfs_inode_size(path->nodes[0], dst_item);
		}
	}

	copy_extent_buffer(path->nodes[0], eb, dst_ptr, src_ptr, item_size);
				      ...
				      struct btrfs_key *key)
{
	...
	if (found_type == BTRFS_FILE_EXTENT_REG ||
		...
		extent_end = ALIGN(start + size, fs_info->sectorsize);
	} else {
		ret = 0;
		goto out;
	}
	...
	if (found_type == BTRFS_FILE_EXTENT_REG ||
         ...
         */
		ret = btrfs_qgroup_trace_extent(
			trans, btrfs_file_extent_disk_bytenr(eb, item),
			btrfs_file_extent_disk_num_bytes(eb, item), GFP_NOFS);
		if (ret < 0)
			goto out;
		...
		if (ins.objectid > 0) {
			...
			if (ret < 0) {
                 ...
                 * allocation tree
                 */
				ret = btrfs_alloc_logged_file_extent(
			...
			while (!list_empty(&ordered_sums)) {
				...
				if (!ret)
					ret = btrfs_del_csums(
						trans, fs_info->csum_root,
						sums->bytenr, sums->len);
				if (!ret)
					ret = btrfs_csum_file_blocks(
						trans, fs_info->csum_root,
						sums);
				list_del(&sums->list);
				kfree(sums);
				      ...
				      struct btrfs_dir_item *di)
{
	...
	ret = btrfs_unlink_inode(trans, dir, BTRFS_I(inode), name, name_len);
	if (ret)
		goto out;
	else
...
static noinline int inode_in_dir(struct btrfs_root *root,
				 struct btrfs_path *path, u64 dirid,
				 u64 objectid, u64 index, const char *name,
				 int name_len)
{
	struct btrfs_dir_item *di;
	struct btrfs_key location;
	...
	di = btrfs_lookup_dir_index_item(NULL, root, path, dirid, index, name,
					 name_len, 0);
	if (IS_ERR(di)) {
		ret = PTR_ERR(di);
		goto out;
...
static noinline int backref_in_log(struct btrfs_root *log,
				   struct btrfs_key *key, u64 ref_objectid,
				   const char *name, int namelen)
{
	...
	if (key->type == BTRFS_INODE_EXTREF_KEY)
		...
		ret = !!btrfs_find_name_in_backref(
			path->nodes[0], path->slots[0], name, namelen);
out:
...
static inline int
__add_inode_ref(struct btrfs_trans_handle *trans, struct btrfs_root *root,
		struct btrfs_path *path, struct btrfs_root *log_root,
		struct btrfs_inode *dir, struct btrfs_inode *inode,
		u64 inode_objectid, u64 parent_objectid, u64 ref_index,
...
again:
	...
	if (!IS_ERR_OR_NULL(extref)) {
		...
		while (cur_offset < item_size) {
			...
			search_key.offset = btrfs_extref_hash(
			...
			if (ret < 0) {
				...
				if (victim_parent) {
					...
					ret = btrfs_unlink_inode(
						trans, BTRFS_I(victim_parent),
						inode, victim_name,
						victim_name_len);
					if (!ret)
	...
	di = btrfs_lookup_dir_item(trans, root, path, btrfs_ino(dir), name,
				   namelen, 0);
	if (IS_ERR(di)) {
		return PTR_ERR(di);
	} else if (di) {
...
static int unlink_old_inode_refs(struct btrfs_trans_handle *trans,
				 ...
				 struct extent_buffer *log_eb, int log_slot,
				 struct btrfs_key *key)
{
	int ret;
...
again:
	...
	while (ref_ptr < ref_end) {
		...
		if (key->type == BTRFS_INODE_EXTREF_KEY)
			ret = !!btrfs_find_name_in_ext_backref(
				log_eb, log_slot, parent_id, name, namelen);
		else
		...
		if (!ret) {
			...
			ret = btrfs_unlink_inode(trans, BTRFS_I(dir), inode,
						 name, namelen);
			kfree(name);
			iput(dir);
			if (ret)
...
static int add_link(struct btrfs_trans_handle *trans, struct inode *dir,
		    ...
		    u64 ref_index)
{
	...
	dir_item = btrfs_lookup_dir_item(
		NULL, root, path, btrfs_ino(BTRFS_I(dir)), name, namelen, 0);
	if (!dir_item) {
		btrfs_release_path(path);
		goto add_link;
	...
	if (ret)
		goto out;
add_link:
	ret = btrfs_add_link(trans, BTRFS_I(dir), BTRFS_I(inode), name, namelen,
			     0, ref_index);
out:
	iput(other_inode);
	btrfs_free_path(path);
 ...
 */
static noinline int
add_inode_ref(struct btrfs_trans_handle *trans, struct btrfs_root *root,
	      struct btrfs_root *log, struct btrfs_path *path,
	      struct extent_buffer *eb, int slot, struct btrfs_key *key)
{
	...
	while (ref_ptr < ref_end) {
		...
		if (ret < 0) {
			...
			if (!search_done) {
				ret = __add_inode_ref(
					trans, root, path, log, BTRFS_I(dir),
					BTRFS_I(inode), inode_objectid,
			       ...
			       struct btrfs_path *path)
{
	...
	while (1) {
		...
		while (cur_offset < item_size) {
			extref =
				(struct btrfs_inode_extref *)(ptr + cur_offset);
...
static int count_inode_refs(struct btrfs_root *root, struct btrfs_inode *inode,
			    struct btrfs_path *path)
{
	int ret;
	struct btrfs_key key;
	...
	process_slot:
		...
		if (key.objectid != ino || key.type != BTRFS_INODE_REF_KEY)
			break;
		...
		while (ptr < ptr_end) {
			...
			name_len =
				btrfs_inode_ref_name_len(path->nodes[0], ref);
					   ...
					   struct inode *inode)
{
	...
	if (inode->i_nlink == 0) {
		if (S_ISDIR(inode->i_mode)) {
			ret = replay_dir_deletes(trans, root, NULL, path, ino,
						 1);
			if (ret)
				goto out;
		}
...
static noinline int insert_one_name(struct btrfs_trans_handle *trans,
				    struct btrfs_root *root, u64 dirid,
 ...
 */
static noinline int
replay_one_name(struct btrfs_trans_handle *trans, struct btrfs_root *root,
		struct btrfs_path *path, struct extent_buffer *eb,
		struct btrfs_dir_item *di, struct btrfs_key *key)
{
	...
	index_dst_di =
		btrfs_lookup_dir_index_item(trans, root, path, key->objectid,
					    key->offset, name, name_len, 1);
	...
	btrfs_release_path(path);
	ret = insert_one_name(trans, root, key->objectid, key->offset, name,
			      name_len, &log_key);
 ...
 */
static noinline int
check_item_in_log(struct btrfs_trans_handle *trans, struct btrfs_root *log,
		  struct btrfs_path *path, struct btrfs_path *log_path,
		  struct inode *dir, struct btrfs_key *dir_key)
{
	struct btrfs_root *root = BTRFS_I(dir)->root;
	int ret;
	struct extent_buffer *eb;
...
static int replay_xattr_deletes(struct btrfs_trans_handle *trans,
				struct btrfs_root *root, struct btrfs_root *log,
				struct btrfs_path *path, const u64 ino)
{
	struct btrfs_key search_key;
				       ...
				       int del_all)
{
	...
	while (1) {
		if (del_all)
			...
			ret = find_dir_range(log, path, dirid, &range_start,
					     &range_end);
			if (ret < 0)
				goto out;
			else if (ret > 0)
		...
		while (1) {
			int nritems;
			ret = btrfs_search_slot(NULL, root, &dir_key, path, 0,
						0);
			if (ret < 0)
				goto out;
			...
			ret = check_item_in_log(trans, log, path, log_path, dir,
						&found_key);
			if (ret)
				goto out;
			     ...
			     struct walk_control *wc, u64 gen, int level)
{
	...
	for (i = 0; i < nritems; i++) {
		...
		if (key.type == BTRFS_INODE_ITEM_KEY &&
			...
			inode_item =
				btrfs_item_ptr(eb, i, struct btrfs_inode_item);
			/*
			...
			ret = replay_xattr_deletes(wc->trans, root, log, path,
						   key.objectid);
			if (ret)
				break;
			mode = btrfs_inode_mode(eb, inode_item);
			if (S_ISDIR(mode)) {
				ret = replay_dir_deletes(wc->trans, root, log,
							 path, key.objectid, 0);
				if (ret)
					break;
			}
			ret = overwrite_item(wc->trans, root, path, eb, i,
					     &key);
			if (ret)
				break;
			...
			ret = link_to_fixup_dir(wc->trans, root, path,
						key.objectid);
			if (ret)
				break;
		}
		...
		if (key.type == BTRFS_DIR_INDEX_KEY &&
			...
			ret = replay_one_dir_item(wc->trans, root, path, eb, i,
						  &key);
			if (ret)
				break;
		}
		...
		if (key.type == BTRFS_XATTR_ITEM_KEY) {
			...
			ret = add_inode_ref(wc->trans, root, log, path, eb, i,
					    &key);
			if (ret && ret != -ENOENT)
				break;
			ret = 0;
		} else if (key.type == BTRFS_EXTENT_DATA_KEY) {
			ret = replay_one_extent(wc->trans, root, path, eb, i,
						&key);
			if (ret)
				break;
		}
				       ...
				       struct walk_control *wc)
{
	...
	while (*level > 0) {
		...
		next = btrfs_find_create_tree_block(
			fs_info, bytenr, btrfs_header_owner(cur), *level - 1);
		if (IS_ERR(next))
			return PTR_ERR(next);
		...
		if (*level == 1) {
			...
			if (wc->free) {
				...
				if (trans) {
					...
					ret = btrfs_pin_reserved_extent(
						trans, bytenr, blocksize);
					if (ret) {
						free_extent_buffer(next);
						return ret;
					}
...
static void wait_log_commit(struct btrfs_root *root, int transid)
{
     ...
     */
	for (;;) {
		prepare_to_wait(&root->log_commit_wait[index], &wait,
 ...
 * can access the list.
 */
static inline void btrfs_remove_all_log_ctxs(struct btrfs_root *root, int index,
					     int error)
{
	struct btrfs_log_ctx *ctx;
		   ...
		   struct btrfs_log_ctx *ctx)
{
	...
	if (atomic_read(&log_root_tree->log_commit[index2])) {
		...
		wait_log_commit(log_root_tree, root_log_ctx.log_transid);
		mutex_unlock(&log_root_tree->log_mutex);
		if (!ret)
			ret = root_log_ctx.log_ret;
		goto out;
			 ...
			 struct btrfs_inode *inode)
{
     ...
     */
	if (inode->logged_trans == 0 && inode->last_trans == trans->transid &&
				  ...
				  u64 index)
{
     ...
     * for dir item keys.
     */
	di = btrfs_lookup_dir_index_item(trans, log, path, dir_ino, index, name,
					 name_len, -1);
	if (IS_ERR(di)) {
		err = PTR_ERR(di);
		goto fail;
...
void btrfs_del_inode_ref_in_log(struct btrfs_trans_handle *trans,
				struct btrfs_root *root, const char *name,
				int name_len, struct btrfs_inode *inode,
				u64 dirid)
{
	struct btrfs_root *log;
	u64 index;
...
static int flush_dir_items_batch(struct btrfs_trans_handle *trans,
				 ...
				 struct btrfs_path *dst_path, int start_slot,
				 int count)
{
	char *ins_data = NULL;
	...
	for (i = path->slots[0]; i < nritems; i++) {
		...
		if (btrfs_dir_transid(src, di) < trans->transid) {
			if (key.offset > *last_old_dentry_offset + 1) {
				ret = insert_dir_log_key(
					trans, log, dst_path, ino,
 ...
 */
static noinline int
log_dir_items(struct btrfs_trans_handle *trans, struct btrfs_inode *inode,
	      struct btrfs_path *path, struct btrfs_path *dst_path,
	      struct btrfs_log_ctx *ctx, u64 min_offset, u64 *last_offset_ret)
{
					  ...
					  struct btrfs_log_ctx *ctx)
{
	...
	while (1) {
		ret = log_dir_items(trans, inode, path, dst_path, ctx, min_key,
				    &max_key);
		if (ret)
			return ret;
		if (max_key == (u64)-1)
...
static int drop_inode_items(struct btrfs_trans_handle *trans,
			    struct btrfs_root *log, struct btrfs_path *path,
			    struct btrfs_inode *inode, int max_key_type)
{
	int ret;
	struct btrfs_key key;
	struct btrfs_key found_key;
...
static void fill_inode_item(struct btrfs_trans_handle *trans,
			    ...
			    int log_inode_only, u64 logged_isize)
{
			  ...
			  struct btrfs_inode *inode, bool inode_item_dropped)
{
	...
	if (!inode_item_dropped && inode->logged_trans == trans->transid) {
		ret = btrfs_search_slot(trans, log, &inode->location, path, 0,
					1);
		ASSERT(ret <= 0);
		if (ret > 0)
			ret = -ENOENT;
	...
	inode_item = btrfs_item_ptr(path->nodes[0], path->slots[0],
				    struct btrfs_inode_item);
	fill_inode_item(trans, path->nodes[0], inode_item, &inode->vfs_inode, 0,
			0);
	btrfs_release_path(path);
	return 0;
}
...
static int log_csums(struct btrfs_trans_handle *trans,
		     struct btrfs_inode *inode, struct btrfs_root *log_root,
		     struct btrfs_ordered_sum *sums)
{
...
static noinline int copy_items(struct btrfs_trans_handle *trans,
			       struct btrfs_inode *inode,
			       struct btrfs_path *dst_path,
			       struct btrfs_path *src_path, int start_slot,
			       int nr, int inode_only, u64 logged_isize)
{
	...
	ins_data = kmalloc(nr * sizeof(struct btrfs_key) + nr * sizeof(u32),
			   GFP_NOFS);
	if (!ins_data)
		return -ENOMEM;
	...
	for (i = 0; i < nr; i++, dst_path->slots[0]++) {
         ...
         * again
         */
		if (ins_keys[i].type == BTRFS_EXTENT_DATA_KEY && !skip_csum) {
			int found_type;
			...
			if (found_type == BTRFS_FILE_EXTENT_REG) {
				u64 ds, dl, cs, cl;
				ds = btrfs_file_extent_disk_bytenr(src, extent);
				/* ds == 0 is a hole */
				if (ds == 0)
					continue;
				...
				cl = btrfs_file_extent_num_bytes(src, extent);
				...
				ret = btrfs_lookup_csums_range(
					fs_info->csum_root, ds + cs,
					ds + cs + cl - 1, &ordered_sums, 0);
				if (ret)
					break;
			}
		}
     ...
     */
	while (!list_empty(&ordered_sums)) {
		struct btrfs_ordered_sum *sums = list_entry(
			ordered_sums.next, struct btrfs_ordered_sum, list);
		if (!ret)
			    ...
			    struct btrfs_log_ctx *ctx)
{
	...
	ret = btrfs_lookup_csums_range(
		trans->fs_info->csum_root, em->block_start + csum_offset,
		em->block_start + csum_offset + csum_len - 1, &ordered_sums, 0);
	if (ret)
		return ret;
...
static int log_one_extent(struct btrfs_trans_handle *trans,
			  struct btrfs_inode *inode,
			  const struct extent_map *em, struct btrfs_path *path,
			  struct btrfs_log_ctx *ctx)
{
	struct btrfs_drop_extents_args drop_args = { 0 };
	struct btrfs_root *log = inode->root->log_root;
	...
	if (em->compress_type != BTRFS_COMPRESS_NONE) {
		btrfs_set_token_file_extent_disk_bytenr(&token, fi,
							em->block_start);
		btrfs_set_token_file_extent_disk_num_bytes(&token, fi,
							   block_len);
	} else if (em->block_start < EXTENT_MAP_LAST_BYTE) {
		btrfs_set_token_file_extent_disk_bytenr(
			&token, fi, em->block_start - extent_offset);
		btrfs_set_token_file_extent_disk_num_bytes(&token, fi,
							   block_len);
	} else {
				      ...
				      struct btrfs_path *path)
{
	...
	while (true) {
		...
		if (WARN_ON_ONCE(key.objectid < ino) ||
		    key.type < BTRFS_EXTENT_DATA_KEY || key.offset < i_size) {
			path->slots[0]++;
			continue;
		}
		if (!dropped_extents) {
	...
	if (ins_nr > 0)
		ret = copy_items(trans, inode, dst_path, path, start_slot,
				 ins_nr, 1, 0);
out:
	btrfs_release_path(path);
...
process:
     ...
     */
	list_for_each_entry_safe (ordered, tmp, &ctx->ordered_extents,
				  log_list) {
		...
		if (!test_bit(BTRFS_ORDERED_COMPLETE, &ordered->flags)) {
			...
			if (!test_bit(BTRFS_ORDERED_COMPLETE,
				...
				atomic_inc(
					&trans->transaction->pending_ordered);
			}
			   ...
			   struct btrfs_inode *inode, struct btrfs_path *path)
{
	...
	if (prev_extent_end < i_size) {
		...
		ret = btrfs_insert_file_extent(trans, root->log_root, ino,
					       prev_extent_end, 0, 0, hole_len,
					       0, hole_len, 0, 0, 0);
		if (ret < 0)
			return ret;
	}
					 ...
					 u64 *other_ino, u64 *other_parent)
{
	...
	while (cur_offset < item_size) {
		...
		if (di && !IS_ERR(di)) {
			...
			btrfs_dir_item_key_to_cpu(search_path->nodes[0], di,
						  &di_key);
...
static int log_conflicting_inodes(struct btrfs_trans_handle *trans,
				  struct btrfs_root *root,
				  struct btrfs_path *path,
				  struct btrfs_log_ctx *ctx, u64 ino,
				  u64 parent)
{
	...
	while (!list_empty(&inode_list)) {
         ...
         */
		ret = btrfs_log_inode(trans, BTRFS_I(inode), LOG_OTHER_INODE,
				      ctx);
		if (ret) {
			btrfs_add_delayed_iput(inode);
			continue;
...
static int
copy_inode_items_to_log(struct btrfs_trans_handle *trans,
			struct btrfs_inode *inode, struct btrfs_key *min_key,
			const struct btrfs_key *max_key,
			struct btrfs_path *path, struct btrfs_path *dst_path,
			const u64 logged_isize, const bool recursive_logging,
			const int inode_only, struct btrfs_log_ctx *ctx,
			bool *need_log_inode_item)
{
	...
	again:
		...
		if ((min_key->type == BTRFS_INODE_REF_KEY ||
		    ...
		    inode->generation == trans->transid && !recursive_logging) {
			u64 other_ino = 0;
			u64 other_parent = 0;

			ret = btrfs_check_ref_name_override(
				path->nodes[0], path->slots[0], min_key, inode,
				&other_ino, &other_parent);
			if (ret < 0) {
				return ret;
			} else if (ret > 0 &&
				   other_ino !=
					   btrfs_ino(BTRFS_I(ctx->inode))) {
				if (ins_nr > 0) {
					ins_nr++;
				} else {
					ins_nr = 1;
			   ...
			   struct btrfs_log_ctx *ctx)
{
	...
	err = copy_inode_items_to_log(trans, inode, &min_key, &max_key, path,
				      dst_path, logged_isize, recursive_logging,
				      inode_only, ctx, &need_log_inode_item);
	if (err)
		goto out_unlock;
...
log_extents:
	...
	if (need_log_inode_item) {
		...
		if (!xattrs_logged && inode->logged_trans < trans->transid) {
			err = btrfs_log_all_xattrs(trans, inode, path,
						   dst_path);
			if (err)
				goto out_unlock;
			btrfs_release_path(path);
				...
				struct btrfs_log_ctx *ctx)
{
	...
	again:
		btrfs_release_path(path);
		ret = btrfs_search_forward(root, &min_key, path,
					   trans->transid);
		if (ret < 0) {
			goto next_dir_inode;
		} else if (ret > 0) {
		...
		for (i = path->slots[0]; i < nritems; i++) {
			...
			btrfs_release_path(path);
			di_inode =
				btrfs_iget(fs_info->sb, di_key.objectid, root);
			if (IS_ERR(di_inode)) {
		...
		while (cur_offset < item_size) {
			...
			if (key.type == BTRFS_INODE_EXTREF_KEY) {
				...
				extref = (struct btrfs_inode_extref
						  *)(ptr + cur_offset);
				inode_key.objectid =
					btrfs_inode_extref_parent(leaf, extref);
				cur_offset += sizeof(*extref);
				cur_offset += btrfs_inode_extref_name_len(
					leaf, extref);
			} else {
			...
			if (!ret && ctx->log_new_dentries)
				ret = log_new_dir_dentries(
					trans, root, BTRFS_I(dir_inode), ctx);
			btrfs_add_delayed_iput(dir_inode);
			if (ret)
				goto out;
		}
				  ...
				  struct btrfs_log_ctx *ctx)
{
	...
	while (true) {
		...
		if (inode->generation >= trans->transid &&
			...
			ret = btrfs_log_inode(trans, inode, LOG_INODE_EXISTS,
					      ctx);
			if (ret)
				break;
		}
...
static int btrfs_log_inode_parent(struct btrfs_trans_handle *trans,
				  struct btrfs_inode *inode,
				  struct dentry *parent, int inode_only,
				  struct btrfs_log_ctx *ctx)
{
	struct btrfs_root *root = inode->root;
	struct btrfs_fs_info *fs_info = root->fs_info;
...
int btrfs_log_dentry_safe(struct btrfs_trans_handle *trans,
			  struct dentry *dentry, struct btrfs_log_ctx *ctx)
{
...
again:
	...
	while (1) {
		...
		wc.replay_dest =
			btrfs_get_fs_root(fs_info, found_key.offset, true);
