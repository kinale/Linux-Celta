...
#define BTRFS_ATTR_PTR(_prefix, _name) (&btrfs_attr_##_prefix##_##_name.attr)
...
#define BTRFS_FEAT_ATTR_PTR(_name) (&btrfs_attr_features_##_name.kobj_attr.attr)
					...
					const char *buf, size_t count)
{
	...
	btrfs_info(fs_info, "%s %s feature flag", val ? "Setting" : "Clearing",
...
static const char *rescue_opts[] = {
	"usebackuproot",   "nologreplay", "ignorebadroots",
	"ignoredatacsums", "all",
};
...
BTRFS_ATTR(static_feature, supported_sectorsizes, supported_sectorsizes_show);
 ...
 */
static struct attribute *btrfs_debug_feature_attrs[] = { NULL };
 ...
 */
static struct attribute *raid_attrs[] = { BTRFS_ATTR_PTR(raid, total_bytes),
...
#define SPACE_INFO_ATTR(field)                                                 \
	static ssize_t btrfs_space_info_show_##field(                          \
...
static ssize_t btrfs_label_show(struct kobject *kobj, struct kobj_attribute *a,
				char *buf)
{
	struct btrfs_fs_info *fs_info = to_fs_info(kobj);
...
static ssize_t btrfs_label_store(struct kobject *kobj, struct kobj_attribute *a,
				 const char *buf, size_t len)
{
	struct btrfs_fs_info *fs_info = to_fs_info(kobj);
	size_t p_len;
						 ...
						 char *buf)
{
	int val;
	struct btrfs_device *device =
		container_of(kobj, struct btrfs_device, devid_kobj);
...
int btrfs_sysfs_add_fsid(struct btrfs_fs_devices *fs_devs)
{
	...
	fs_devs->devices_kobj =
		kobject_create_and_add("devices", &fs_devs->fsid_kobj);
	if (!fs_devs->devices_kobj) {
	...
	fs_devs->devinfo_kobj =
		kobject_create_and_add("devinfo", &fs_devs->fsid_kobj);
	if (!fs_devs->devinfo_kobj) {
		btrfs_err(fs_devs->fs_info,
...
int btrfs_sysfs_add_mounted(struct btrfs_fs_info *fs_info)
{
	...
	error = sysfs_create_group(fsid_kobj, &btrfs_feature_attr_group);
	if (error)
		goto failure;
	...
	/* Discard directory */
	fs_info->discard_debug_kobj =
		kobject_create_and_add("discard", fs_info->debug_kobj);
	if (!fs_info->discard_debug_kobj) {
		error = -ENOMEM;
		goto failure;
	}
	...
	fs_info->space_info_kobj =
		kobject_create_and_add("allocation", fsid_kobj);
	if (!fs_info->space_info_kobj) {
		error = -ENOMEM;
		goto failure;
	}
...
#define QGROUP_ATTR(_member, _show_name)                                       \
	static ssize_t btrfs_qgroup_show_##_member(                            \
		struct kobject *qgroup_kobj, struct kobj_attribute *a,         \
		char *buf)                                                     \
	{                                                                      \
		struct btrfs_fs_info *fs_info =                                \
			qgroup_kobj_to_fs_info(qgroup_kobj);                   \
		struct btrfs_qgroup *qgroup =                                  \
			container_of(qgroup_kobj, struct btrfs_qgroup, kobj);  \
...
#define QGROUP_RSV_ATTR(_name, _type)                                          \
	static ssize_t btrfs_qgroup_rsv_show_##_name(                          \
		struct kobject *qgroup_kobj, struct kobj_attribute *a,         \
		char *buf)                                                     \
	{                                                                      \
		struct btrfs_fs_info *fs_info =                                \
...
{
	struct btrfs_qgroup *qgroup =
		container_of(kobj, struct btrfs_qgroup, kobj);
 ...
 */
void btrfs_sysfs_feature_update(struct btrfs_fs_info *fs_info, u64 bit,
				enum btrfs_feature_set set)
{
