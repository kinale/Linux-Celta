			    ...
			    void *bitmap)
{
	...
	entry->type =
		(bitmap) ? BTRFS_FREE_SPACE_BITMAP : BTRFS_FREE_SPACE_EXTENT;
...
int load_free_space_cache(struct btrfs_block_group *block_group)
{
	...
	ret = __load_free_space_cache(fs_info->tree_root, inode, &tmp_ctl, path,
				      block_group->start);
	btrfs_free_path(path);
	if (ret <= 0)
		goto out;

	matched = (tmp_ctl.free_space ==
		   (block_group->length - used - block_group->bytes_super));
...
write_cache_extent_entries(struct btrfs_io_ctl *io_ctl,
			   struct btrfs_free_space_ctl *ctl,
			   struct btrfs_block_group *block_group, int *entries,
			   int *bitmaps, struct list_head *bitmap_list)
{
	int ret;
	...
	if (block_group && !list_empty(&block_group->cluster_list)) {
		cluster =
			list_entry(block_group->cluster_list.next,
...
static noinline_for_stack int
update_cache_item(struct btrfs_trans_handle *trans, struct btrfs_root *root,
		  struct inode *inode, struct btrfs_path *path, u64 offset,
...
static noinline_for_stack int
write_pinned_extent_entries(struct btrfs_trans_handle *trans,
			    struct btrfs_block_group *block_group,
			    struct btrfs_io_ctl *io_ctl, int *entries)
{
	...
	while (start < block_group->start + block_group->length) {
		ret = find_first_extent_bit(unpin, start, &extent_start,
...
cleanup_write_cache_enospc(struct inode *inode, struct btrfs_io_ctl *io_ctl,
			...
			struct btrfs_path *path)
{
	return __btrfs_wait_cache_io(block_group->fs_info->tree_root, trans,
				     block_group, &block_group->io_ctl, path,
				     block_group->start);
}
				   ...
				   struct btrfs_trans_handle *trans)
{
	...
	spin_lock(&ctl->tree_lock);
	ret = write_cache_extent_entries(io_ctl, ctl, block_group, &entries,
					 &bitmaps, &bitmap_list);
	if (ret)
...
tree_search_offset(struct btrfs_free_space_ctl *ctl, u64 offset,
		   int bitmap_only, int fuzzy)
{
	...
	while (1) {
		if (entry->bitmap) {
			if (entry->offset + BITS_PER_BITMAP * ctl->unit >
			    offset)
				break;
		} else {
...
static inline void __unlink_free_space(struct btrfs_free_space_ctl *ctl,
				       struct btrfs_free_space *info)
{
...
again:
	...
	*bytes -= search_bytes;
	...
	if (*bytes) {
		...
		bitmap_info =
			rb_entry(next, struct btrfs_free_space, offset_index);
...
no_cluster_bitmap:
	bitmap_info =
		tree_search_offset(ctl, offset_to_bitmap(ctl, offset), 1, 0);
	if (!bitmap_info) {
		ASSERT(added == 0);
		goto new_bitmap;
	}
...
int __btrfs_add_free_space(struct btrfs_fs_info *fs_info,
			   struct btrfs_free_space_ctl *ctl, u64 offset,
			   u64 bytes, enum btrfs_trim_state trim_state)
{
	struct btrfs_block_group *block_group = ctl->private;
	struct btrfs_free_space *info;
					...
					u64 bytenr, u64 size, bool used)
{
	...
	const int bg_reclaim_threshold =
...
int btrfs_add_free_space(struct btrfs_block_group *block_group, u64 bytenr,
			 u64 size)
{
	...
	return __btrfs_add_free_space(block_group->fs_info,
				      block_group->free_space_ctl, bytenr, size,
				      trim_state);
}
...
int btrfs_remove_free_space(struct btrfs_block_group *block_group, u64 offset,
			    u64 bytes)
{
	struct btrfs_free_space_ctl *ctl = block_group->free_space_ctl;
	struct btrfs_free_space *info;
...
again:
	...
	if (!info) {
         ...
         */
		info = tree_search_offset(ctl, offset_to_bitmap(ctl, offset), 1,
					  0);
		if (!info) {
			/*
 ...
 */
static void
__btrfs_return_cluster_to_free_space(struct btrfs_block_group *block_group,
				     struct btrfs_free_cluster *cluster)
{
	struct btrfs_free_space_ctl *ctl = block_group->free_space_ctl;
	struct btrfs_free_space *entry;
...
out:
	...
	if (align_gap_len)
		__btrfs_add_free_space(block_group->fs_info, ctl, align_gap,
				       align_gap_len, align_gap_trim_state);
	return ret;
}
 ...
 */
void btrfs_return_cluster_to_free_space(struct btrfs_block_group *block_group,
					struct btrfs_free_cluster *cluster)
{
	struct btrfs_free_space_ctl *ctl;
...
static u64 btrfs_alloc_from_bitmap(struct btrfs_block_group *block_group,
				   struct btrfs_free_cluster *cluster,
				   struct btrfs_free_space *entry, u64 bytes,
				   u64 min_start, u64 *max_extent_size)
{
	...
	if (err) {
		*max_extent_size =
			max(get_max_extent_size(entry), *max_extent_size);
		return 0;
	}
			     ...
			     u64 min_start, u64 *max_extent_size)
{
	...
	while (1) {
		...
		if (entry->bitmap) {
			ret = btrfs_alloc_from_bitmap(block_group, cluster,
						      entry, bytes,
...
static int btrfs_bitmap_cluster(struct btrfs_block_group *block_group,
				struct btrfs_free_space *entry,
				struct btrfs_free_cluster *cluster, u64 offset,
				u64 bytes, u64 cont1_bytes, u64 min_bytes)
{
	struct btrfs_free_space_ctl *ctl = block_group->free_space_ctl;
	unsigned long next_zero;
	unsigned long i;
...
again:
	...
	for_each_set_bit_from (i, entry->bitmap, BITS_PER_BITMAP) {
		next_zero =
			find_next_zero_bit(entry->bitmap, BITS_PER_BITMAP, i);
	...
	trace_btrfs_setup_cluster(block_group, cluster, total_found * ctl->unit,
				  1);
	return 0;
}
...
static noinline int setup_cluster_bitmap(struct btrfs_block_group *block_group,
					 struct btrfs_free_cluster *cluster,
					 struct list_head *bitmaps, u64 offset,
					 u64 bytes, u64 cont1_bytes,
					 u64 min_bytes)
{
	struct btrfs_free_space_ctl *ctl = block_group->free_space_ctl;
	struct btrfs_free_space *entry = NULL;
...
int btrfs_trim_block_group(struct btrfs_block_group *block_group, u64 *trimmed,
			   u64 start, u64 end, u64 minlen)
{
	struct btrfs_free_space_ctl *ctl = block_group->free_space_ctl;
	int ret;
	u64 rem = 0;
				       ...
				       struct btrfs_trans_handle *trans)
{
	...
	while (node) {
		block_group =
			rb_entry(node, struct btrfs_block_group, cache_node);
 ...
 * configurations.
 */
int test_add_free_space_entry(struct btrfs_block_group *cache, u64 offset,
			      u64 bytes, bool bitmap)
{
...
again:
	...
	if (!map) {
		map = kmem_cache_zalloc(btrfs_free_space_bitmap_cachep,
					GFP_NOFS);
		if (!map) {
			kmem_cache_free(btrfs_free_space_cachep, info);
			return -ENOMEM;
