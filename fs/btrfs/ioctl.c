...
#define BTRFS_IOC_SEND_32                                                      \
	_IOW(BTRFS_IOCTL_MAGIC, 38, struct btrfs_ioctl_send_args_32)
#endif
...
{
	if (flags & ~(FS_IMMUTABLE_FL | FS_APPEND_FL | FS_NOATIME_FL |
		      FS_NODUMP_FL | FS_SYNC_FL | FS_DIRSYNC_FL | FS_NOCOMP_FL |
...
int btrfs_fileattr_set(struct user_namespace *mnt_userns, struct dentry *dentry,
		       struct fileattr *fa)
{
	...
	if (comp) {
		...
		ret = btrfs_set_prop(trans, inode, "btrfs.compression", NULL, 0,
				     0);
		if (ret && ret != -ENODATA) {
			btrfs_abort_transaction(trans, ret);
			goto out_end_trans;
				  ...
				  struct btrfs_qgroup_inherit *inherit)
{
	...
	btrfs_set_stack_inode_nbytes(inode_item, fs_info->nodesize);
	...
	ret = btrfs_insert_root(trans, fs_info->tree_root, &key, root_item);
	if (ret) {
		/*
			   ...
			   struct btrfs_qgroup_inherit *inherit)
{
	...
	if (ret < 0)
		goto free_pending;
	pending_snapshot->root_item =
		kzalloc(sizeof(struct btrfs_root_item), GFP_KERNEL);
     ...
     * 1 - UUID item
     */
	ret = btrfs_subvolume_reserve_metadata(
		BTRFS_I(dir)->root, &pending_snapshot->block_rsv, 8, false);
	if (ret)
		goto free_pending;
...
static noinline int btrfs_mksubvol(const struct path *parent,
				   ...
				   const char *name, int namelen,
				   struct btrfs_root *snap_src, bool readonly,
				   struct btrfs_qgroup_inherit *inherit)
{
     ...
     */
	error = btrfs_check_dir_item_collision(BTRFS_I(dir)->root, dir->i_ino,
					       name, namelen);
	if (error)
		goto out_dput;
	...
	if (snap_src)
		...
		error = create_subvol(mnt_userns, dir, dentry, name, namelen,
				      inherit);
...
static noinline int btrfs_mksnapshot(const struct path *parent,
				     struct user_namespace *mnt_userns,
				     const char *name, int namelen,
				     struct btrfs_root *root, bool readonly,
				     struct btrfs_qgroup_inherit *inherit)
{
	int ret;
	...
	ret = btrfs_mksubvol(parent, mnt_userns, name, namelen, root, readonly,
			     inherit);
out:
 ...
 */
static int defrag_collect_targets(struct btrfs_inode *inode, u64 start, u64 len,
				  u32 extent_thresh, u64 newer_than,
				  bool do_compress, bool locked,
				  struct list_head *target_list)
{
	...
	while (cur < start + len) {
		...
		next_mergeable =
			defrag_check_next_extent(&inode->vfs_inode, em, locked);
		if (!next_mergeable) {
	...
	list_for_each_entry (entry, &target_list, list) {
		...
		if (max_sectors)
			range_len = min_t(u32, range_len,
					  (max_sectors - *sectors_defragged) *
						  sectorsize);

		if (ra)
			page_cache_sync_readahead(
				inode->vfs_inode.i_mapping, ra, NULL,
		      ...
		      u64 newer_than, unsigned long max_to_defrag)
{
	...
	while (cur < last_byte) {
		...
		cluster_end = (((cur >> PAGE_SHIFT) + (SZ_256K >> PAGE_SHIFT))
			       << PAGE_SHIFT) -
			      1;
...
static noinline int btrfs_ioctl_resize(struct file *file, void __user *arg)
{
	...
	ret = exclop_start_or_cancel_reloc(fs_info, BTRFS_EXCLOP_RESIZE,
					   cancel);
	if (ret)
		goto out_free;
	/* Exclusive operation is now claimed */
	...
	if (ret == 0 && new_size != old_size)
		btrfs_info_in_rcu(
			...
			rcu_str_deref(device->name), device->devid, old_size,
			new_size);
out_finish:
...
static noinline int
__btrfs_ioctl_snap_create(struct file *file, struct user_namespace *mnt_userns,
			  ...
			  bool readonly, struct btrfs_qgroup_inherit *inherit)
{
	...
	if (subvol) {
		ret = btrfs_mksubvol(&file->f_path, mnt_userns, name, namelen,
		...
		if (src_inode->i_sb != file_inode(file)->i_sb) {
			...
			ret = btrfs_mksnapshot(&file->f_path, mnt_userns, name,
					       namelen,
...
static noinline int btrfs_ioctl_snap_create(struct file *file, void __user *arg,
					    int subvol)
{
	struct btrfs_ioctl_vol_args *vol_args;
	int ret;
...
static noinline int copy_to_sk(struct btrfs_path *path, struct btrfs_key *key,
			       struct btrfs_ioctl_search_key *sk,
			       size_t *buf_size, char __user *ubuf,
			       unsigned long *sk_offset, int *num_found)
{
	u64 found_transid;
	struct extent_buffer *leaf;
				 ...
				 size_t *buf_size, char __user *ubuf)
{
	...
	while (1) {
		...
		ret = copy_to_sk(path, &key, sk, buf_size, ubuf, &sk_offset,
				 &num_found);
...
static int
btrfs_search_path_in_tree_user(struct user_namespace *mnt_userns,
			       struct inode *inode,
			       struct btrfs_ioctl_ino_lookup_user_args *args)
{
	...
	if (dirid != upper_limit.objectid) {
		...
		while (1) {
			...
			*(ptr + len) = '/';
			read_extent_buffer(leaf, ptr, (unsigned long)(iref + 1),
					   len);
...
static int btrfs_ioctl_ino_lookup_user(struct file *file, void __user *argp)
{
	...
	ret = btrfs_search_path_in_tree_user(file_mnt_user_ns(file), inode,
					     args);
...
static int btrfs_ioctl_get_subvol_info(struct file *file, void __user *argp)
{
	...
	if (key.objectid != BTRFS_FS_TREE_OBJECTID) {
		...
		if (key.objectid == subvol_info->treeid &&
			...
			item_len = btrfs_item_size_nr(leaf, slot) -
				   sizeof(struct btrfs_root_ref);
			read_extent_buffer(leaf, subvol_info->name, item_off,
					   item_len);
		} else {
			ret = -ENOENT;
			goto out;
...
static noinline int btrfs_ioctl_snap_destroy(struct file *file,
					     void __user *arg, bool destroy_v2)
{
...
static int btrfs_ioctl_defrag(struct file *file, void __user *argp)
{
	...
	case S_IFREG:
		...
		ret = btrfs_defrag_file(file_inode(file), &file->f_ra, &range,
					BTRFS_OLDEST_GENERATION, 0);
		if (ret > 0)
			ret = 0;
		break;
	default:
...
static long btrfs_ioctl_rm_dev_v2(struct file *file, void __user *arg)
{
	...
	if (vol_args->flags & BTRFS_DEVICE_SPEC_BY_ID) {
		...
		ret = btrfs_get_dev_args_from_path(fs_info, &args,
						   vol_args->name);
		if (ret)
			goto out;
	}
...
static long btrfs_ioctl_default_subvol(struct file *file, void __user *argp)
{
	...
	di = btrfs_lookup_dir_item(trans, fs_info->tree_root, path, dir_id,
				   "default", 7, 1);
				   ...
				   void __user *arg)
{
	...
	static const u64 types[] = { BTRFS_BLOCK_GROUP_DATA,
...
static long
_btrfs_ioctl_set_received_subvol(struct file *file,
				 struct user_namespace *mnt_userns,
				 struct btrfs_ioctl_received_subvol_args *sa)
{
	...
	received_uuid_changed =
		memcmp(root_item->received_uuid, sa->uuid, BTRFS_UUID_SIZE);
					       ...
					       void __user *arg)
{
	...
	ret = _btrfs_ioctl_set_received_subvol(file, file_mnt_user_ns(file),
					       args64);
	if (ret)
		goto out;
...
static long btrfs_ioctl_set_received_subvol(struct file *file, void __user *arg)
{
...
int btrfs_ioctl_get_supported_features(void __user *arg)
{
	static const struct btrfs_ioctl_feature_flags features[3] = {
		INIT_FEATURE_FLAGS(SUPP), INIT_FEATURE_FLAGS(SAFE_SET),
...
static int check_feature_bits(struct btrfs_fs_info *fs_info,
			      enum btrfs_feature_set set, u64 change_mask,
			      u64 flags, u64 supported_flags, u64 safe_set,
			      u64 safe_clear)
{
...
static int _btrfs_ioctl_send(struct file *file, void __user *argp, bool compat)
{
	...
	if (compat) {
		...
		memcpy(arg->reserved, args32.reserved, sizeof(args32.reserved));
#else
		return -ENOTTY;
#endif
	} else {
