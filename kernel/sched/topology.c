...
#define SD_FLAG(_name, mflags)                                                 \
	[__##_name] = { .meta_flags = mflags, .name = #_name },
				  ...
				  struct cpumask *groupmask)
{
	...
	do {
		...
		printk(KERN_CONT " %d:{ span=%*pbl", group->sgc->id,
		       cpumask_pr_args(sched_group_span(group)));

		if ((sd->flags & SD_OVERLAP) &&
		    !cpumask_equal(group_balance_mask(group),
...
static struct sched_group *get_group(int cpu, struct sd_data *sdd)
{
	...
	sg->sgc->capacity =
		SCHED_CAPACITY_SCALE * cpumask_weight(sched_group_span(sg));
	sg->sgc->min_capacity = SCHED_CAPACITY_SCALE;
	sg->sgc->max_capacity = SCHED_CAPACITY_SCALE;
...
static struct sched_domain *build_sched_domain(
	struct sched_domain_topology_level *tl, const struct cpumask *cpu_map,
	struct sched_domain_attr *attr, struct sched_domain *child, int cpu)
{
			       ...
			       struct sched_domain_attr *attr)
{
	...
	for_each_cpu (i, cpu_map) {
		...
		if (rq->cpu_capacity_orig > READ_ONCE(d.rd->max_cpu_capacity))
			WRITE_ONCE(d.rd->max_cpu_capacity,
