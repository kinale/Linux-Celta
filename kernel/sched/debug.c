...
{
	static const char *preempt_modes[] = { "none", "voluntary", "full" };
	int i;
...
static __init int sched_init_debug(void)
{
	...
	debugfs_create_u32("idle_min_granularity_ns", 0644, debugfs_sched,
			   &sysctl_sched_idle_min_granularity);
	debugfs_create_u32("wakeup_granularity_ns", 0644, debugfs_sched,
...
#define P_SCHEDSTAT(F)                                                         \
	SEQ_printf(m, "  .%-30s: %lld\n", #F,                                  \
		   (long long)schedstat_val(stats->F))
#define PN(F)                                                                  \
	SEQ_printf(m, "  .%-30s: %lld.%06ld\n", #F, SPLIT_NS((long long)F))
#define PN_SCHEDSTAT(F)                                                        \
	SEQ_printf(m, "  .%-30s: %lld.%06ld\n", #F,                            \
		   SPLIT_NS((long long)schedstat_val(stats->F)))
...
static void print_task(struct seq_file *m, struct rq *rq, struct task_struct *p)
{
	...
	SEQ_printf(m, " %15s %5d %9Ld.%06ld %9Ld %5d ", p->comm, task_pid_nr(p),
		   SPLIT_NS(p->se.vruntime), (long long)(p->nvcsw + p->nivcsw),
		   p->prio);
...
static const char *sched_tunable_scaling_names[] = { "none", "logarithmic",
						     "linear" };
