...
static inline void update_sched_rq_watermark(struct rq *rq)
{
	unsigned long watermark =
		find_first_bit(rq->queue.bitmap, SCHED_QUEUE_BITS);
...
static inline struct task_struct *sched_rq_first_task(struct rq *rq)
{
	...
	const struct list_head *head =
		&rq->queue.heads[sched_prio2idx(idx, rq)];
...
static inline void __task_access_unlock(struct task_struct *p,
					raw_spinlock_t *lock)
{
	if (NULL != lock)
...
static inline struct rq *task_access_lock_irqsave(struct task_struct *p,
						  raw_spinlock_t **plock,
						  unsigned long *flags)
{
	struct rq *rq;
...
static inline void task_access_unlock_irqrestore(struct task_struct *p,
						 raw_spinlock_t *lock,
						 unsigned long *flags)
{
...
#define RQ_LOAD_HISTORY_TO_UTIL(l)                                             \
	(((l) >> (RQ_LOAD_HISTORY_BITS - 1 - RQ_UTIL_SHIFT)) & 0xff)
...
{
	return RQ_LOAD_HISTORY_TO_UTIL(rq->load_history) *
	       (max >> RQ_UTIL_SHIFT);
}
...
static inline void dequeue_task(struct task_struct *p, struct rq *rq, int flags)
{
	...
	WARN_ONCE(task_rq(p) != rq,
		  "sched: dequeue task reside on cpu%d from cpu%d\n",
...
static inline void enqueue_task(struct task_struct *p, struct rq *rq, int flags)
{
	...
	WARN_ONCE(task_rq(p) != rq,
		  "sched: enqueue task reside on cpu%d to cpu%d\n", task_cpu(p),
		  cpu_of(rq));
...
static inline void requeue_task(struct task_struct *p, struct rq *rq)
{
	...
	WARN_ONCE(task_rq(p) != rq,
		  "sched: cpu[%d] requeue task reside on cpu%d\n", cpu_of(rq),
		  task_cpu(p));
...
static void __do_set_cpus_ptr(struct task_struct *p,
...
void sched_set_stop_task(int cpu, struct task_struct *stop)
{
	...
	if (old_stop) {
         ...
         */
		sched_setscheduler_nocheck(old_stop, SCHED_NORMAL,
					   &start_param);
	}
}
				  ...
				  const struct cpumask *new_mask, u32 flags)
{
	...
	return __set_cpus_allowed_ptr_locked(p, new_mask, flags, rq, lock,
					     irq_flags);
}
				     ...
				     const struct cpumask *subset_mask)
{
	...
	return __set_cpus_allowed_ptr_locked(p, new_mask, 0, rq, lock,
					     irq_flags);
 ...
 */
static inline void ttwu_do_wakeup(struct rq *rq, struct task_struct *p,
				  int wake_flags)
{
	check_preempt_curr(rq);
...
static inline void ttwu_do_activate(struct rq *rq, struct task_struct *p,
				    int wake_flags)
{
...
static bool ttwu_queue_wakelist(struct task_struct *p, int cpu, int wake_flags)
{
	if (__is_defined(ALT_SCHED_TTWU_QUEUE) &&
	    ttwu_queue_cond(cpu, wake_flags)) {
 ...
 */
static inline void __sched_fork(unsigned long clone_flags,
...
static inline int active_load_balance_cpu_stop(void *data)
{
	...
	if (task_on_rq_queued(p) && task_rq(p) == rq &&
		...
		int cpu = cpu_of(rq);
		int dcpu =
			__best_mask_cpu(&tmp, per_cpu(sched_cpu_llc_mask, cpu));
...
static inline int sg_balance_trigger(const int cpu)
{
	...
	if (res)
		stop_one_cpu_nowait(cpu, active_load_balance_cpu_stop, curr,
...
{
	printk(KERN_INFO
	       "sched: pending: 0x%04lx, idle: 0x%04lx, sg_idle: 0x%04lx\n",
	       sched_rq_pending_mask.bits[0], sched_rq_watermark[0].bits[0],
 ...
 */
static inline int migrate_pending_tasks(struct rq *rq, struct rq *dest_rq,
					const int dest_cpu)
{
...
static inline int take_other_rq_tasks(struct rq *rq, int cpu)
{
	...
	do {
		...
		for_each_cpu_and (i, &sched_rq_pending_mask, topo_mask) {
			...
			if ((nr_migrated =
				     migrate_pending_tasks(src_rq, rq, cpu))) {
				src_rq->nr_running -= nr_migrated;
				if (src_rq->nr_running < 2)
					cpumask_clear_cpu(
						i, &sched_rq_pending_mask);
...
static void sched_init_topology_cpumask(void)
{
	...
	for_each_online_cpu (cpu) {
		...
		printk(KERN_INFO
		       "sched: cpu#%02d llc_id = %d, llc_mask idx = %d\n",
