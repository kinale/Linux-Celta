...
static int posix_cpu_clock_getres(const clockid_t which_clock,
...
static int posix_cpu_clock_set(const clockid_t clock,
			       ...
			       struct itimerspec64 *old)
{
     ...
     */
	timer->it_requeue_pending =
		(timer->it_requeue_pending + 2) & ~REQUEUE_PENDING;
...
static void posix_cpu_timer_get(struct k_itimer *timer,
				    ...
				    struct list_head *firing)
{
	...
	for (i = 0; i < CPUCLOCK_MAX; i++, base++) {
		base->nextevt =
			collect_timerqueue(&base->tqhead, firing, samples[i]);
	}
}
			     ...
			     u64 *expires, u64 cur_time, int signo)
{
	...
	if (cur_time >= it->expires) {
		...
		trace_itimer_expire(signo == SIGPROF ? ITIMER_PROF :
							     ITIMER_VIRTUAL,
 ...
 */
static inline bool task_cputimers_expired(const u64 *samples,
