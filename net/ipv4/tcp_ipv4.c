...
void tcp_ld_RTO_revert(struct sock *sk, u32 seq)
{
	...
	if (remaining > 0) {
		inet_csk_reset_xmit_timer(sk, ICSK_TIME_RETRANS, remaining,
					  TCP_RTO_MAX);
	} else {
...
static void tcp_v4_send_reset(const struct sock *sk, struct sk_buff *skb)
{
	...
	ip_send_unicast_reply(ctl_sk, skb, &TCP_SKB_CB(skb)->header.h4.opt,
			      ...
			      arg.iov[0].iov_len, transmit_time);
...
static int tcp_v4_parse_md5_keys(struct sock *sk, int optname, sockptr_t optval,
				 int optlen)
{
				    ...
				    int sdif)
{
	...
	if (genhash || memcmp(hash_location, newhash, 16) != 0) {
		...
		net_info_ratelimited(
			...
			ntohs(th->dest),
			genhash ? " tcp_v4_calc_md5_hash failed" : "", l3index);
		return true;
	}
	return false;
#endif
...
bool tcp_add_backlog(struct sock *sk, struct sk_buff *skb)
{
	...
	if (TCP_SKB_CB(tail)->end_seq != TCP_SKB_CB(skb)->seq ||
	    ...
	    ((TCP_SKB_CB(tail)->tcp_flags | TCP_SKB_CB(skb)->tcp_flags) &
	     (TCPHDR_SYN | TCPHDR_RST | TCPHDR_URG)) ||
	    !((TCP_SKB_CB(tail)->tcp_flags & TCP_SKB_CB(skb)->tcp_flags) &
	      TCPHDR_ACK) ||
	    ((TCP_SKB_CB(tail)->tcp_flags ^ TCP_SKB_CB(skb)->tcp_flags) &
