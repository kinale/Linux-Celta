...
static void mptcp_set_datafin_timeout(const struct sock *sk)
{
	...
	mptcp_sk(sk)->timer_ival =
		min(TCP_RTO_MAX, TCP_RTO_MIN << icsk->icsk_retransmits);
}
...
static long
mptcp_timeout_from_subflow(const struct mptcp_subflow_context *subflow)
{
	const struct sock *ssk = mptcp_subflow_tcp_sock(subflow);

	return inet_csk(ssk)->icsk_pending && !subflow->stale_count ?
		       inet_csk(ssk)->icsk_timeout - jiffies :
			     0;
}
...
static void mptcp_set_timeout(struct sock *sk)
{
	...
	mptcp_for_each_subflow(mptcp_sk(sk), subflow) tout =
		max(tout, mptcp_timeout_from_subflow(subflow));
...
static bool mptcp_subflow_could_cleanup(const struct sock *ssk, bool rx_empty)
{
	...
	return (ack_pending & ICSK_ACK_SCHED) &&
		...
		(rx_empty &&
		 ack_pending & (ICSK_ACK_PUSHED2 | ICSK_ACK_PUSHED)));
}
...
static bool mptcp_do_flush_join_list(struct mptcp_sock *msk)
{
	...
	list_for_each_entry (subflow, &msk->join_list, node) {
		...
		mptcp_propagate_sndbuf((struct sock *)msk,
...
{
	bool sync_needed =
		test_and_clear_bit(MPTCP_WORK_SYNC_SETSOCKOPT, &msk->flags);
...
static bool mptcp_skb_can_collapse_to(u64 write_seq, const struct sk_buff *skb,
...
{
	int reclaimable =
		mptcp_sk(sk)->rmem_fwd_alloc - sk_unused_reserved_mem(sk);
...
static void __mptcp_clean_una(struct sock *sk)
{
	...
	if (unlikely(msk->recovery) &&
	    after64(msk->snd_una, msk->recovery_snd_nxt))
		msk->recovery = false;
...
static void mptcp_update_data_checksum(struct sk_buff *skb, int added)
{
	...
	mpext->csum = csum_fold(csum_block_add(
		csum, skb_checksum(skb, offset, added, 0), offset));
}
			      ...
			      struct mptcp_sendmsg_info *info)
{
	...
	pr_debug(
		"msk=%p ssk=%p sending dfrag at seq=%llu len=%u already sent=%u",
...
#define MPTCP_SEND_BURST_SIZE                                                  \
	((1 << 16) - sizeof(struct tcphdr) - MAX_TCP_OPTION_SPACE -            \
	 sizeof(struct ipv6hdr) - sizeof(struct frag_hdr))
...
bool mptcp_subflow_active(struct mptcp_subflow_context *subflow)
{
	if (unlikely(subflow->stale)) {
		u32 rcv_tstamp = READ_ONCE(
			tcp_sk(mptcp_subflow_tcp_sock(subflow))->rcv_tstamp);
...
static void mptcp_update_post_push(struct mptcp_sock *msk,
				   struct mptcp_data_frag *dfrag, u32 sent)
{
...
static void __mptcp_subflow_push_pending(struct sock *sk, struct sock *ssk)
{
	...
	while ((dfrag = mptcp_send_head(sk))) {
		...
		while (len > 0) {
             ...
             */
			xmit_ssk = first ? ssk :
						 mptcp_subflow_get_send(mptcp_sk(sk));
			if (!xmit_ssk)
				goto out;
			if (xmit_ssk != ssk) {
				mptcp_subflow_delegate(
					mptcp_subflow_ctx(xmit_ssk),
					MPTCP_DELEGATE_SEND);
				goto out;
			}
...
static void mptcp_rcv_space_adjust(struct mptcp_sock *msk, int copied)
{
	...
	if (sock_net(sk)->ipv4.sysctl_tcp_moderate_rcvbuf &&
		...
		grow = rcvwin *
		       (msk->rcvq_space.copied - msk->rcvq_space.space);
...
out_err:
	...
	pr_debug("msk=%p rx queue empty=%d:%d copied=%d", msk,
		 skb_queue_empty_lockless(&sk->sk_receive_queue),
...
static struct sock *mptcp_subflow_get_retrans(struct mptcp_sock *msk)
{
	...
	mptcp_for_each_subflow(msk, subflow)
	{
		...
		if (!tcp_rtx_and_write_queues_empty(ssk)) {
			...
			min_stale_count = min_t(int, min_stale_count,
						subflow->stale_count);
			continue;
		}
		     ...
		     struct mptcp_subflow_context *subflow)
{
	if (sk->sk_state == TCP_ESTABLISHED)
		mptcp_event(MPTCP_EVENT_SUB_CLOSED, mptcp_sk(sk), ssk,
			    GFP_KERNEL);
...
static void __mptcp_wr_shutdown(struct sock *sk)
{
	...
	pr_debug(
		"msk=%p snd_data_fin_enable=%d shutdown=%x state=%d pending=%d",
...
{
	unsigned int offset =
		sizeof(struct mptcp6_sock) - sizeof(struct ipv6_pinfo);
...
void mptcp_rcv_space_init(struct mptcp_sock *msk, const struct sock *ssk)
{
	...
	msk->rcvq_space.space =
		min_t(u32, tp->rcv_wnd, TCP_INIT_CWND * tp->advmss);
				...
				int addr_len, int flags)
{
	...
	if (subflow->request_mptcp && mptcp_token_new_connect(ssock->sk)) {
		MPTCP_INC_STATS(sock_net(ssock->sk),
			   ...
			   struct poll_table_struct *wait)
{
	...
	if (state == TCP_LISTEN)
		return test_bit(MPTCP_DATA_READY, &msk->flags) ?
			       EPOLLIN | EPOLLRDNORM :
				     0;
...
void __init mptcp_proto_init(void)
{
	...
	for_each_possible_cpu (cpu) {
		...
		netif_tx_napi_add(&mptcp_napi_dev, &delegated->napi,
				  mptcp_napi_poll, NAPI_POLL_WEIGHT);
	...
	BUILD_BUG_ON(sizeof(struct mptcp_skb_cb) >
