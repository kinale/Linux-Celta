			   ...
			   size_t count, loff_t *ppos)
{
	...
	return simple_read_from_buffer(userbuf, count, ppos, ref->ops->name,
				       len);
}
...
static bool rc_no_data_or_no_ack_use_min(struct ieee80211_tx_rate_control *txrc)
{
	...
	return (info->flags &
		(IEEE80211_TX_CTL_NO_ACK | IEEE80211_TX_CTL_USE_MINRATE)) ||
...
{
	int i;
	u32 rate_flags = ieee80211_chandef_rate_flags(&hw->conf.chandef);
	...
	WARN_ONCE(
		...
		sta ? sta->addr : NULL, sta ? sta->supp_rates[sband->band] : -1,
		sband->band, rate_mask, rate_flags);

	info->control.rates[0].count = (info->flags & IEEE80211_TX_CTL_NO_ACK) ?
					       1 :
						     hw->max_rate_tries;
...
static void rate_idx_match_mask(s8 *rate_idx, u16 *rate_flags,
				struct ieee80211_supported_band *sband,
				enum nl80211_chan_width chan_width, u32 mask,
				...
				struct ieee80211_tx_rate *rates, int max_rates)
{
	...
	for (i = 0; i < max_rates; i++) {
		...
		if (rates[i].flags & IEEE80211_TX_RC_VHT_MCS) {
			if (unlikely(ieee80211_rate_get_vht_mcs(&rates[i]) > 9))
				pr_warn("%s: ieee80211_rate_get_vht_mcs(&rates[%d]) == %d > 9\n",
					__func__, i,
					ieee80211_rate_get_vht_mcs(&rates[i]));
			continue;
		}
		...
		if (unlikely(rates[i].idx >= sband->n_bitrates)) {
			pr_warn_once(
				"%s: rates[%d].idx == %d >= sband->n_bitrates == %d\n",
...
void ieee80211_get_tx_rates(struct ieee80211_vif *vif,
			    ...
			    struct ieee80211_tx_rate *dest, int max_rates)
{
