...
#define MCS_SYMBOL_TIME(sgi, syms)                                             \
	(sgi   ? ((syms)*18000 + 4000) / 5 : /* syms * 3.6 us */               \
...
#define GROUP_IDX(_streams, _sgi, _ht40)                                       \
	MINSTREL_HT_GROUP_0 + MINSTREL_MAX_STREAMS * 2 * _ht40 +               \
		MINSTREL_MAX_STREAMS *_sgi + _streams - 1
...
#define __MCS_GROUP(_streams, _sgi, _ht40, _s)                                 \
	[GROUP_IDX(_streams, _sgi, _ht40)] = {                                 \
		...
		.duration = { MCS_DURATION(_streams, _sgi, _ht40 ? 54 : 26) >> \
				      _s,                                      \
...
#define VHT_GROUP_IDX(_streams, _sgi, _bw)                                     \
	(MINSTREL_VHT_GROUP_0 + MINSTREL_MAX_STREAMS * 2 * (_bw) +             \
	 MINSTREL_MAX_STREAMS * (_sgi) + (_streams)-1)

#define BW2VBPS(_bw, r3, r2, r1) (_bw == BW_80 ? r3 : _bw == BW_40 ? r2 : r1)
	...
	[VHT_GROUP_IDX(_streams, _sgi, _bw)] = {                               \
		...
		.flags = IEEE80211_TX_RC_VHT_MCS |                             \
			 ...
			 (_bw == BW_80 ? IEEE80211_TX_RC_80_MHZ_WIDTH :        \
			  _bw == BW_40 ? IEEE80211_TX_RC_40_MHZ_WIDTH :        \
					       0),                                   \
		.duration = { MCS_DURATION(_streams, _sgi,                     \
...
#define VHT_GROUP_SHIFT(_streams, _sgi, _bw)                                   \
	GROUP_SHIFT(MCS_DURATION(_streams, _sgi, BW2VBPS(_bw, 117, 54, 26)))

#define VHT_GROUP(_streams, _sgi, _bw)                                         \
	__VHT_GROUP(_streams, _sgi, _bw, VHT_GROUP_SHIFT(_streams, _sgi, _bw))

#define CCK_DURATION(_bitrate, _short)                                         \
	(1000 * (10 /* SIFS */ + (_short ? 72 + 24 : 144 + 48) +               \
		 (8 * (AVG_PKT_SIZE + 4) * 10) / (_bitrate)))

#define CCK_DURATION_LIST(_short, _s)                                          \
	CCK_DURATION(10, _short) >> _s, CCK_DURATION(20, _short) >> _s,        \
		CCK_DURATION(55, _short) >> _s,                                \
		CCK_DURATION(110, _short) >> _s

#define __CCK_GROUP(_s)                                                        \
	[MINSTREL_CCK_GROUP] = { .streams = 1,                                 \
				 .flags = 0,                                   \
				 .shift = _s,                                  \
				 .duration = { CCK_DURATION_LIST(false, _s),   \
					       CCK_DURATION_LIST(true, _s) } }

#define CCK_GROUP_SHIFT GROUP_SHIFT(CCK_DURATION(10, false))

#define CCK_GROUP __CCK_GROUP(CCK_GROUP_SHIFT)

#define OFDM_DURATION(_bitrate)                                                \
	(1000 * (16 /* SIFS + signal ext */ + 16 /* T_PREAMBLE */ +            \
		 4 /* T_SIGNAL */ +                                            \
		 4 * (((16 + 80 * (AVG_PKT_SIZE + 4) + 6) / ((_bitrate)*4)))))

#define OFDM_DURATION_LIST(_s)                                                 \
	OFDM_DURATION(60) >> _s, OFDM_DURATION(90) >> _s,                      \
		OFDM_DURATION(120) >> _s, OFDM_DURATION(180) >> _s,            \
		OFDM_DURATION(240) >> _s, OFDM_DURATION(360) >> _s,            \
		OFDM_DURATION(480) >> _s, OFDM_DURATION(540) >> _s

#define __OFDM_GROUP(_s)                                                       \
	[MINSTREL_OFDM_GROUP] = { .streams = 1,                                \
				  .flags = 0,                                  \
				  .shift = _s,                                 \
				  .duration = {                                \
...
#define OFDM_GROUP_SHIFT GROUP_SHIFT(OFDM_DURATION(60))
...
static const u8 minstrel_sample_seq[] = {
	MINSTREL_SAMPLE_TYPE_INC, MINSTREL_SAMPLE_TYPE_JUMP,
	MINSTREL_SAMPLE_TYPE_INC, MINSTREL_SAMPLE_TYPE_JUMP,
	MINSTREL_SAMPLE_TYPE_INC, MINSTREL_SAMPLE_TYPE_SLOW,
};

static void minstrel_ht_update_rates(struct minstrel_priv *mp,
 ...
 */
static u16 minstrel_get_valid_vht_rates(int bw, int nss, __le16 mcs_map)
{
	u16 mask = 0;
...
static bool minstrel_ht_is_legacy_group(int group)
{
	return group == MINSTREL_CCK_GROUP || group == MINSTREL_OFDM_GROUP;
}
...
static int minstrel_vht_get_group_idx(struct ieee80211_tx_rate *rate)
{
...
static inline int minstrel_get_duration(int index)
{
	const struct mcs_group *group =
		&minstrel_mcs_groups[MI_RATE_GROUP(index)];
	unsigned int duration = group->duration[MI_RATE_IDX(index)];

	return duration << group->shift;
}

static unsigned int minstrel_ht_avg_ampdu_len(struct minstrel_ht_sta *mi)
{
	int duration;
 ...
 */
static void minstrel_ht_sort_best_tp_rates(struct minstrel_ht_sta *mi,
					   u16 index, u16 *tp_list)
{
	...
	if (j < MAX_THR_RATES - 1) {
		memmove(&tp_list[j + 1], &tp_list[j],
			(sizeof(*tp_list) * (MAX_THR_RATES - (j + 1))));
	}
	if (j < MAX_THR_RATES)
		tp_list[j] = index;
}
 ...
 */
static void minstrel_ht_set_best_prob_rate(struct minstrel_ht_sta *mi,
					   u16 *dest, u16 index)
{
	...
	if (mrs->prob_avg > MINSTREL_FRAC(75, 100)) {
		...
		max_gpr_tp_avg = minstrel_ht_get_tp_avg(
			mi, max_gpr_group, max_gpr_idx, max_gpr_prob);
				 ...
				 u16 tmp_legacy_tp_rate[MAX_THR_RATES])
{
	...
	if (tmp_cck_tp > tmp_mcs_tp) {
		for (i = 0; i < MAX_THR_RATES; i++) {
			minstrel_ht_sort_best_tp_rates(
				mi, tmp_legacy_tp_rate[i], tmp_mcs_tp_rate);
		}
	}
}
...
minstrel_ht_prob_rate_reduce_streams(struct minstrel_ht_sta *mi)
{
	...
	for (group = 0; group < ARRAY_SIZE(minstrel_mcs_groups); group++) {
		...
		if (tmp_tp < minstrel_ht_get_tp_avg(mi, group, tmp_idx,
			...
			tmp_tp = minstrel_ht_get_tp_avg(mi, group, tmp_idx,
							tmp_prob);
		}
	}
}

static u16 __minstrel_ht_get_sample_rate(struct minstrel_ht_sta *mi,
...
*/
static void minstrel_ht_calc_rate_stats(struct minstrel_priv *mp,
					struct minstrel_rate_stats *mrs)
{
	...
	if (unlikely(mrs->attempts > 0)) {
		...
		minstrel_filter_avg_add(&mrs->prob_avg, &mrs->prob_avg_1,
					cur_prob);
...
static bool minstrel_ht_find_sample_rate(struct minstrel_ht_sta *mi, int type,
					 int idx)
{
	int i;
...
static int minstrel_ht_move_sample_rates(struct minstrel_ht_sta *mi, int type,
...
static int minstrel_ht_group_min_rate_offset(struct minstrel_ht_sta *mi,
					     int group, u32 max_duration)
{
 ...
 */
static u16 minstrel_ht_next_inc_rate(struct minstrel_ht_sta *mi,
				     u32 fast_rate_dur)
{
...
static int minstrel_ht_next_group_sample_rate(struct minstrel_ht_sta *mi,
					      int group, u16 supported,
					      int offset)
{
 ...
 */
static u16 minstrel_ht_next_jump_rate(struct minstrel_ht_sta *mi,
				      u32 fast_rate_dur, u32 slow_rate_dur,
				      int *slow_rate_ofs)
{
	struct minstrel_rate_stats *mrs;
...
static void minstrel_ht_refill_sample_rates(struct minstrel_ht_sta *mi)
{
	...
	for (i = 0; i < ARRAY_SIZE(mi->sample); i++)
		memcpy(mi->sample[i].cur_sample_rates,
		       mi->sample[i].sample_rates,
...
static bool minstrel_ht_txstat_valid(struct minstrel_priv *mp,
				     struct minstrel_ht_sta *mi,
				     struct ieee80211_tx_rate *rate)
{
	int i;
...
static void minstrel_downgrade_rate(struct minstrel_ht_sta *mi, u16 *idx,
				    bool primary)
{
				  ...
				  struct ieee80211_tx_status *st)
{
	...
	if (mp->hw->max_rates > 1) {
		...
		if (rate->attempts > 30 && rate->success < rate->attempts / 4) {
				 ...
				 int offset, int index)
{
	...
	if (group_idx == MINSTREL_CCK_GROUP)
		...
		idx = mp->ofdm_rates[mi->band]
				    [index % ARRAY_SIZE(mp->ofdm_rates[0])];
	else if (flags & IEEE80211_TX_RC_VHT_MCS)
		idx = ((group->streams - 1) << 4) | (index & 0xF);
	else
...
static inline int minstrel_ht_get_prob_avg(struct minstrel_ht_sta *mi, int rate)
{
...
static int minstrel_ht_get_max_amsdu_len(struct minstrel_ht_sta *mi)
{
...
static u16 minstrel_ht_get_sample_rate(struct minstrel_priv *mp,
				       struct minstrel_ht_sta *mi)
{
	u8 seq;
...
static void minstrel_ht_get_rate(void *priv, struct ieee80211_sta *sta,
				 void *priv_sta,
...
static void minstrel_ht_update_cck(struct minstrel_priv *mp,
				   struct minstrel_ht_sta *mi,
				   struct ieee80211_supported_band *sband,
...
static void minstrel_ht_update_ofdm(struct minstrel_priv *mp,
				    struct minstrel_ht_sta *mi,
				    struct ieee80211_supported_band *sband,
				    struct ieee80211_sta *sta)
{
				    ...
				    struct ieee80211_sta *sta, void *priv_sta)
{
	...
	ack_dur =
		ieee80211_frame_duration(sband->band, 10, ctl_rate->bitrate,
					 erp, 1,
	...
	for (i = 0; i < ARRAY_SIZE(mi->groups); i++) {
		...
		mi->supported[i] = minstrel_get_valid_vht_rates(
			bw, nss, vht_cap->vht_mcs.tx_mcs_map);
...
static void *minstrel_ht_alloc_sta(void *priv, struct ieee80211_sta *sta,
				   gfp_t gfp)
{
	struct ieee80211_supported_band *sband;
	struct minstrel_ht_sta *mi;
...
static void minstrel_ht_free_sta(void *priv, struct ieee80211_sta *sta,
				 void *priv_sta)
{
	kfree(priv_sta);
}

static void minstrel_ht_fill_rate_array(u8 *dest,
					struct ieee80211_supported_band *sband,
...
static void minstrel_ht_init_cck_rates(struct minstrel_priv *mp)
{
	...
	minstrel_ht_fill_rate_array(mp->cck_rates, sband, minstrel_cck_bitrates,
				    ARRAY_SIZE(minstrel_cck_bitrates),
				    rate_flags);
}

static void minstrel_ht_init_ofdm_rates(struct minstrel_priv *mp,
