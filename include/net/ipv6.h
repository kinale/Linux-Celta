...
#define _DEVINC(net, statname, mod, idev, field)                               \
	({                                                                     \
		...
		if (likely(_idev != NULL))                                     \
			mod##SNMP_INC_STATS64((_idev)->stats.statname,         \
					      (field));                        \
...
#define _DEVINCATOMIC(net, statname, mod, idev, field)                         \
	({                                                                     \
		struct inet6_dev *_idev = (idev);                              \
		if (likely(_idev != NULL))                                     \
			SNMP_INC_STATS_ATOMIC_LONG(                            \
				(_idev)->stats.statname##dev, (field));        \
		mod##SNMP_INC_STATS((net)->mib.statname##_statistics,          \
				    (field));                                  \
	})
	...
	({                                                                     \
		...
		if (likely(_idev != NULL))                                     \
			mod##SNMP_UPD_PO_STATS((_idev)->stats.statname, field, \
					       (val));                         \
		mod##SNMP_UPD_PO_STATS((net)->mib.statname##_statistics,       \
				       field, (val));                          \
	})
...
#define __IP6_INC_STATS(net, idev, field) _DEVINC(net, ipv6, __, idev, field)
...
#if defined(CONFIG_HAVE_EFFICIENT_UNALIGNED_ACCESS) && BITS_PER_LONG == 64
static inline bool
__ipv6_prefix_equal64_half(const __be64 *a1, const __be64 *a2, unsigned int len)
{
				     ...
				     unsigned int prefixlen)
{
	...
	if (prefixlen >= 64) {
		if (a1[0] ^ a2[0])
			return false;
		return __ipv6_prefix_equal64_half(a1 + 1, a2 + 1,
						  prefixlen - 64);
	}
...
{
	return net->ipv6.sysctl.ip_nonlocal_bind || inet->freebind ||
	       inet->transparent;
}
...
static inline int __ip6_sock_set_addr_preferences(struct sock *sk, int val)
{
	...
	switch (val & (IPV6_PREFER_SRC_PUBLIC | IPV6_PREFER_SRC_TMP |
