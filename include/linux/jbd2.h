...
#define JBD2_KNOWN_INCOMPAT_FEATURES                                           \
	(JBD2_FEATURE_INCOMPAT_REVOKE | JBD2_FEATURE_INCOMPAT_64BIT |          \
	 JBD2_FEATURE_INCOMPAT_ASYNC_COMMIT | JBD2_FEATURE_INCOMPAT_CSUM_V2 |  \
...
#define __journal_expect(expr, why...)                                         \
	({                                                                     \
		int val = (expr);                                              \
		if (!val) {                                                    \
			printk(KERN_ERR "JBD2 unexpected failure: %s: %s;\n",  \
			       __func__, #expr);                               \
			printk(KERN_ERR why "\n");                             \
		}                                                              \
		val;                                                           \
...
static inline unsigned long jbd2_time_diff(unsigned long start,
					   unsigned long end)
{
	if (end >= start)
		return end - start;
...
struct journal_s {
	...
	int (*j_fc_replay_callback)(struct journal_s *journal,
				    struct buffer_head *bh, enum passtype pass,
				    int off, tid_t expected_commit_id);
};
...
#define JBD2_ABORT_ON_SYNCDATA_ERR                                             \
	0x040 /* Abort the journal on file
...
#define JBD2_JOURNAL_FLUSH_VALID                                               \
	(JBD2_JOURNAL_FLUSH_DISCARD | JBD2_JOURNAL_FLUSH_ZEROOUT)

/*
 * Journal atomic flag definitions
 */
#define JBD2_CHECKPOINT_IO_ERROR                                               \
	0x001 /* Detect io error while writing
						 * buffer back to disk */
...
extern void __jbd2_journal_file_buffer(struct journal_head *, transaction_t *,
				       int);
extern void __journal_free_buffer(struct journal_head *bh);
extern void jbd2_journal_file_buffer(struct journal_head *, transaction_t *,
				     int);
extern void __journal_clean_data_list(transaction_t *transaction);
static inline void jbd2_file_log_bh(struct list_head *head,
...
extern void
jbd2_buffer_frozen_trigger(struct journal_head *jh, void *mapped_data,
			   struct jbd2_buffer_trigger_type *triggers);
extern void
jbd2_buffer_abort_trigger(struct journal_head *jh,
			  struct jbd2_buffer_trigger_type *triggers);
...
extern int jbd2__journal_restart(handle_t *, int nblocks, int revoke_records,
				 gfp_t gfp_mask);
extern int jbd2_journal_start_reserved(handle_t *handle, unsigned int type,
...
extern int jbd2_journal_invalidatepage(journal_t *, struct page *, unsigned int,
				       unsigned int);
...
extern int jbd2_journal_check_used_features(journal_t *, unsigned long,
					    unsigned long, unsigned long);
extern int jbd2_journal_check_available_features(journal_t *, unsigned long,
						 unsigned long, unsigned long);
extern int jbd2_journal_set_features(journal_t *, unsigned long, unsigned long,
				     unsigned long);
extern void jbd2_journal_clear_features(journal_t *, unsigned long,
					unsigned long, unsigned long);
...
extern int jbd2_journal_update_sb_log_tail(journal_t *, tid_t, unsigned long,
					   int);
...
extern int jbd2_journal_inode_ranged_wait(handle_t *handle,
					  struct jbd2_inode *inode,
					  loff_t start_byte, loff_t length);
extern int jbd2_journal_submit_inode_data_buffers(struct jbd2_inode *jinode);
extern int jbd2_journal_finish_inode_data_buffers(struct jbd2_inode *jinode);
extern int jbd2_journal_begin_ordered_truncate(journal_t *journal,
					       struct jbd2_inode *inode,
					       loff_t new_size);
extern void jbd2_journal_init_jbd_inode(struct jbd2_inode *jinode,
					struct inode *inode);
extern void jbd2_journal_release_jbd_inode(journal_t *journal,
...
static inline unsigned long jbd2_log_space_left(journal_t *journal)
{
	...
	if (journal->j_committing_transaction) {
		free -= atomic_read(&journal->j_committing_transaction
...
static inline u32 jbd2_chksum(journal_t *journal, u32 crc, const void *address,
			      unsigned int length)
{
	struct {
