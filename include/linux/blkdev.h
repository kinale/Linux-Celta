...
#define QUEUE_FLAG_MQ_DEFAULT                                                  \
	((1 << QUEUE_FLAG_IO_STAT) | (1 << QUEUE_FLAG_SAME_COMP) |             \
	 (1 << QUEUE_FLAG_SAME_FORCE) | (1 << QUEUE_FLAG_NOWAIT))
...
extern void
blk_queue_max_zone_append_sectors(struct request_queue *q,
				  unsigned int max_zone_append_sectors);
...
static inline bool blk_needs_flush_plug(struct task_struct *tsk)
{
	...
	return plug && (plug->mq_list || !list_empty(&plug->cb_list));
}
...
queue_max_discard_segments(const struct request_queue *q)
{
...
queue_max_zone_append_sectors(const struct request_queue *q)
{
...
#ifdef CONFIG_COMPAT
extern int blkdev_compat_ptr_ioctl(struct block_device *, fmode_t, unsigned int,
				   unsigned long);
#else
