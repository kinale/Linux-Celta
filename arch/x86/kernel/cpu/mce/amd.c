...
static const char *const th_names[] = {
	"load_store",  "insn_fetch",  "combined_unit",
	"decode_unit", "northbridge", "execution_unit",
};

static const char *const smca_umc_block_names[] = { "dram_ecc", "misc_umc" };
...
static int prepare_threshold_block(unsigned int bank, unsigned int block,
				   u32 addr, int offset, u32 misc_high)
{
...
{
	_log_error_bank(bank, mca_msr_reg(bank, MCA_STATUS),
...
static void amd_threshold_interrupt(void)
{
	...
	for (bank = 0; bank < this_cpu_read(mce_num_banks); ++bank) {
		...
		list_for_each_entry_safe (block, tmp, &first_block->miscj,
					  miscj)
...
static ssize_t show_error_count(struct threshold_block *b, char *buf)
{
	...
	return sprintf(
		buf, "%u\n",
		((hi & THRESHOLD_MAX) - (THRESHOLD_MAX - b->threshold_limit)));
}
...
static const char *get_name(unsigned int bank, struct threshold_block *b)
{
	...
	snprintf(buf_mcatype, MAX_MCATYPE_NAME_LEN, "%s_%x",
		 smca_get_name(bank_type), smca_banks[bank].sysfs_id);
	return buf_mcatype;
}
				     ...
				     u32 address)
{
	...
	err = kobject_init_and_add(&b->kobj, &threshold_ktype, tb->kobj,
				   get_name(bank, b));
	if (err)
		goto out_free;
recurse:
				 ...
				 unsigned int bank)
{
	...
	err = allocate_threshold_blocks(cpu, b, bank, 0,
					mca_msr_reg(bank, MCA_MISC));
	if (err)
		goto out_kobj;
